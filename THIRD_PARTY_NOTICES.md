...
**biocommons/hgvs-rs (projection corpus)**
- Files: `tests/fixtures/hgvs-rs-projection/cases.json`
- Source (proximate): https://github.com/biocommons/hgvs-rs
- License: Apache License 2.0
- Attribution: biocommons contributors, https://github.com/biocommons/hgvs-rs
- Citation: A Rust port of biocommons/hgvs; the `(HGVSg, HGVSc, HGVSp)` mapper
  test triples trace to the biocommons UTA (Universal Transcript Archive).
  See Hart RK, et al. A Python package for parsing, validating, mapping, and
  formatting sequence variants using HGVS nomenclature. Bioinformatics. 2015.
- Note: `cases.json` is a curated, stratified subset of the upstream
  `tests/data/mapper/*.tsv` corpus, regenerated by
  `scripts/refresh-hgvs-rs-projection-fixtures.py` from a pinned commit; the full
  corpus is not redistributed. The vendored hgvs-rs checkout ships only a
  `LICENSE.txt` (no separate upstream `NOTICE`), so there is no Apache-2.0 §4(d)
  NOTICE to carry forward.
