...
**39 adjudication records — 36 decided, 3 open.**
...
| `mosaic-chimeric-substitution-reference-allele-first` | decided | [`docs/recommendations/DNA/substitution.md:49`](https://github.com/HGVSnomenclature/hgvs-nomenclature/blob/565b9734a4a44fa9bfaa6264e3a3ff092e5e82af/docs/recommendations/DNA/substitution.md?plain=1#L49) | A two-member mosaic (`/`) or chimeric (`//`) substitution allele is written reference-allele-first, so `c.85T>C/=` becomes `c.85=/T>C`, because the spec always describes the reference first; the variant-first spelling is reordered, not refused. |
