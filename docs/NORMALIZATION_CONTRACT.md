...
**44 adjudication records — 41 decided, 3 open.**
...
`ruled`.** That sentence is not written by hand: the generator reads the arm out of
...
| `codon-carve-out-excludes-the-compensated-zero-width-gap-two-pair` | decided | [`docs/recommendations/DNA/delins.md:17`](https://github.com/HGVSnomenclature/hgvs-nomenclature/blob/565b9734a4a44fa9bfaa6264e3a3ff092e5e82af/docs/recommendations/DNA/delins.md?plain=1#L17) | A point insertion and a single-base deletion that share one codon but sit two positions apart are described individually, because the codon exception reaches only changes separated by one nucleotide. |
...
| `codon-exception-reaches-the-termination-codon` | decided | [`docs/recommendations/DNA/delins.md:18`](https://github.com/HGVSnomenclature/hgvs-nomenclature/blob/565b9734a4a44fa9bfaa6264e3a3ff092e5e82af/docs/recommendations/DNA/delins.md?plain=1#L18) | The codon exception applies to the termination codon as to any other: two changes inside a stop codon are written as one delins, because Ter is a described residue position. |
| `codon-exception-vs-coincidence-carve-out-precedence` | decided | house choice (cites none) | On a coding axis, when the codon exception and the payload-coincidence carve-out reach the same pieces and prescribe different merges, ferro applies the codon exception first; both forms are conformant, so this is the project's choice among them. |
...
| `payload-coincidence-separation-and-budget-are-house-choices` | decided | house choice (cites none) | The payload-coincidence carve-out's separation limit and mismatch budget are the project's choice, not the spec's: every value swept yields the same resulting sequence, the budget of 1 is forced by the codon-first precedence, and the separation floor of 5 comes from the spec's own worked delins example. |
...
| `pure-tandem-expansion-insertion-is-not-repartitioned` | decided | house choice (cites none) | A pure insertion that is itself a tandem repeat expansion (or a clean single-copy dup) of its abutting reference tract is emitted as that one member and never re-partitioned into a `[sub; dup]` whose substitution lands on a base no minimal alignment changes; the ruled peel's Part B defers on it, on every axis alike. |
...
| `separation-zero-dup-member-is-preserved-not-merged` | decided | [`docs/recommendations/DNA/duplication.md:18`](https://github.com/HGVSnomenclature/hgvs-nomenclature/blob/565b9734a4a44fa9bfaa6264e3a3ff092e5e82af/docs/recommendations/DNA/duplication.md?plain=1#L18) | At separation zero a tandem-dup member exposed by re-derivation is kept, not merged into a delins, because a change describable as a duplication must be described as one; this knowingly deviates from the spec's own worked delins example, which is base-indistinguishable from the many dup-beside-change cases the rule governs. |
