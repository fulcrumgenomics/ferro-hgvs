...
- `reference_unavailable { reason, tracking_issue }` — the prepared manifest lacks the reference this row needs (an exact pinned accession.version absent from a vanilla `ferro prepare`, or an Ensembl accession absent from the RefSeq-only manifest), so ferro no-ops (echoes the input) and cannot match upstream. Explicitly **not** a ferro defect — it is a provisioning gap tracked by `tracking_issue`, distinct from `known_bug` (which asserts ferro is wrong on a reference it *can* resolve). XPASS-guarded like `known_bug`/`improvement`: when the reference is provisioned and ferro matches, the harness fails loudly to demote the row. `reason` is a closed enum (`accession-version-absent`, `ensembl-absent-from-refseq-manifest`).
...
- #325's definition of done is restated: the umbrella closes when the corpus is fully annotated (every divergence is an `accepted_divergence`, a `known_bug`/`improvement`/`reference_unavailable` with an owning issue, or a documentary `spec_citation`) and the gating test is hermetic — not when a flat file reaches a hand-counted row total.
