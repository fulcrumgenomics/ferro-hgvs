...
This repo uses a set of "oracles" as an extra correctness check. They validate every normalization
output in debug builds, in CI and in local runs. Each oracle is switched on by setting its
`FERRO_ASSERT_*` flag. Read this page when an oracle fires on your PR, when you add a
normalization path, or when you change the oracle jobs in `ci.yml`.
