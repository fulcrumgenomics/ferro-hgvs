...
flags. CI's `test-oracle` job selects the same profile and narrows it on its own command line: it
shards the run with `--partition`, and its `-E` excludes the proptests, sweeps and censuses that
other jobs own (`not test(proptest)`, `SWEEP_FILTER`, `CENSUS_FILTER`). Those are scheduling and
ownership filters, not oracle policy — the profile owns the exclusions and the flags. A local run
applies neither, so it includes tests that job does not.
