...
Two ledger records reach this page, and they differ in `r.` jurisdiction.
`whole-span-reverse-complement-types-as-inv` governs the whole-span reverse-complement → `inv`
typing at `:5`, but it is grounded entirely on DNA clauses and guarded only on the coding and
genomic axes — it carries no `r.` test, so it is not an `r.`-jurisdiction ruling on its own terms.
`RNA/inversion.md:5` states the identical whole-span definition on the RNA axis's own authority, so
the record could be grounded on `r.` without stretching a DNA clause; that extension is a proposed
ledger change, not one made, and is surfaced here for the typing rule rather than claimed as
settled `r.` conformance. `inverted-duplication-is-derived-as-ins-range-inv`, by contrast, cites
`RNA/insertion.md:18` and `RNA/duplication.md:21` directly, so it carries `r.` authority on its own
molecule-native basis and governs `:19`. Other clauses here carry no Why block and are
...
inverted-duplication derivation only on the genomic axis; the `c.`/`n.`/`r.`/`m.` axes still emit
literals. So on `r.` ferro preserves an already-spelled `r.NNN_NNNinv` (or `ins<range>inv`) input,
but does not mint an `inv` from a reverse-complement `delins`, nor keep an `ins<range>inv` range
payload — it expands the range to literal bases. This is an implementation gap, not a spec
question, and changes no verdict below: an already-`inv` input is a fixed point, and a
literal-expanded `ins<range>inv` output is valid HGVS that re-parses.

Executable rows use `NM_004006.3`, the one transcript in the committed slice. The spec's own Notes
and Examples sit on bare illustrative sequences or a foreign accession the slice doesn't carry, so
those spellings are shown on the slice accession or left parse-only (`—`). Both well-formed
inversions used below are measured fixed points on the slice, not merely code-derived ones.
...
`whole-span-reverse-complement-types-as-inv` is grounded on DNA clauses only — no `r.` guard
exists, though `RNA/inversion.md:5` states the identical definition on RNA's own authority, so
extending it would need no DNA-clause stretch (see the page notes above). Per the derivation gap,
ferro preserves an already-spelled `r.` inversion but does not mint one from a reverse-complement
span.
...
|---|---|---|---|
| `NM_004006.3:r.177_180inv` | recommended | self | well-formed 4-nt inversion on the slice; already `inv`, so a fixed point (the spec's own example uses illustrative bases) |
...
analogue is grounded only on DNA clauses (see `:5`). The behavior is nonetheless
CONFIRM-by-inspection: the grammar forecloses the lone-`inv` spelling before any redirect is
needed. Treat this as open/provisional — conformant by construction, not recorded on its own
authority.
...
| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `NM_004006.3:r.177_180inv` | recommended | self | no equivalent 3' placement over the flank, so the 3'rule has nothing to shift; already 3'-most |
...
not by stretching a DNA clause. Per the derivation gap above, ferro expands an already-spelled
...
Ferro: the spec's worked example; its reverse-complement math checks out. A four-nucleotide
whole-span reverse complement types as one `inv` (`whole-span-reverse-complement-types-as-inv`,
cited at `:5`). The spec's spelling is accession-less, not table-executable; the executable twin on
the slice is a fixed point.

| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `NM_004006.3:r.177_180inv` | recommended | self | executable twin of the spec's bare `r.177_180inv`; already `inv` and 3'-most, a fixed point |
...
| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `NM_004006.3:r.203_506inv` | recommended | self | executable twin of the spec's bare `r.203_506inv`; a fixed point |
...
Ferro: descriptive history — the deprecated `o` strand marker is gone; opposite-strand insert
sequence is spelled with an `inv`-suffixed payload (`delins[…inv]`). This concerns
insertion-payload orientation, not inversion typing, and the cited spelling is on a foreign
accession — no executable row.

See also → `RNA/insertion.md:18` and `RNA/duplication.md:18-21` (the same `ins<range>inv` form),
`RNA/substitution.md` (where a one-nucleotide "inversion" lands, per `:16-17`).
