...
These descriptions are genomic, sitting on chromosome (`NC_`), `chr`, `NG_` or foreign accessions
the committed reference slice does not carry — so every verdict row on this page is parse-only
(`Normalizes to` = `—`): the verdict is what ferro's strict parse does with the string, read
without reference bases. Refusals keyed on specific ISCN spellings (comma-thousands coordinates,
the removed cross-chromosome `::`, the `dupinv` form) are the parser's own grammar rejections, in
every mode; the spec text is unambiguous enough that a rejection is plainly conformant, so no
dedicated ledger record is needed for them.
...
Ferro: "complex" is the residual category — what remains when no basic variant type fits. It is
load-bearing elsewhere: it is precisely because a ring chromosome has no single-type description
that the self-cancelling prohibition does not reach across its `::` junctions (see
`complex.md:130`). Descriptive.
...
Ferro: background framing. ISCN describes the resulting chromosome; HGVS describes the variants
detected. The `:13` warning — that a complex description can be "literally correct" yet
"effectively meaningless" — is why a structural rejection rule is not minted from worked examples
plus biology alone (see the open anchoring question at `complex.md:28`). Descriptive.
...
comma-thousands separators is an ISCN rendering, not valid HGVS, and is refused — the parser fails
at the first comma, in every mode.

| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `chr2:g.pter_8,247,756::chr11:g.15,825,273_cen_qter` | refused | — | the ISCN rendering of the balanced `t(2;11)` (`complex.md:75`): comma-thousands coordinates on a `chr` accession; not an HGVS string. Rejected at parse (the first comma), in every mode |
...
(translocation/transposition) use was removed. The spec recommends translocations "exclusively" as
`delins[ACC:g…]` — the adjacent delins row is the HGVS-proper form, and the cross-chromosome `::`
string is not (`class="invalid"` at `complex.md:72`). Ferro accepts the `delins` form and refuses
the cross-chromosome `::` composite at parse, in every mode — `::` is admitted only between
same-accession segments (the ring form at `complex.md:130`), so a second accession after it is a
grammar error.

| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `NC_000002.12:g.pter_8247756delins[NC_000011.10:g.pter_15825272]` | recommended | — | the balanced `t(2;11)` as the spec's `delins` form (`complex.md:70`); parse-only |
...
Ferro: `::` carries connectivity that `;` does not. Two `del` segments joined by `::` form a ring (a
fused-break topology); the same two joined by `;` are two independent deletions. A `::` composite
is therefore not reducible to its member set, so `general.md:57`'s self-cancelling prohibition —
which can only redirect to a competing single-type description — cannot reach across a ring's `::`
junctions, because a ring (complex by definition, `complex.md:5`) has no such single-type
description. Ferro accepts ring composites.
...
| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `NC_000022.11:g.pter_(12200001_14700000)del::(37600001_41000000)_qterdel` | recommended | — | the ring-chromosome form (`complex.md:127`): two `del` segments joined by `::`, which `;` cannot express (parse-only) |
...
open question is whether a ring's first `::` segment must start at `pter` and its last end at
`qter`, so a ring naming only interior coordinates would be refused. The spec publishes only
anchored rings and biology agrees a ring loses both telomeres — but no clause states the
requirement, and `complex.md:13` counsels against reading a rejection rule out of worked examples
alone. **This is undecided.** Ferro accepts an unanchored ring today — the status quo, not a ruling
either way.

<details class="ss-why"><summary>Why ferro reads it this way</summary>

<!-- why:START -->
> **[ring-telomere-anchoring](https://github.com/fulcrumgenomics/ferro-hgvs/blob/main/docs/NORMALIZATION_CONTRACT.md)** — Must a ring chromosome's first `::` segment start at `pter` and its last end at `qter`, so that a ring whose segments name only interior coordinates is refused?
...
Ferro: the ISCN-structural placement/ordering conventions. `:50` restates the general 3' rule for
breakpoint placement; `:51`, `:53` and `:55` fix segment order (`pter` to `qter`) and forward
orientation. Consistent with ferro's existing behaviour; effectively descriptive on this page.
...
naming the source span — never `dupinv`, which is `class="invalid"` (`inversion.md:19`; primary
authority `inversion.md:69`). Ferro derives the `ins<range>inv` form rather than expanding the
payload to literal reverse-complement bases, and refuses `dupinv`.
...
| Input | Verdict | Normalizes to | Notes |
|---|---|---|---|
| `NC_000008.11:g.(127300001_131500000)_(131500001_136400000)dup` | recommended | — | the same-orientation tandem duplication (`complex.md:134`); parse-only |
...
| `NC_000008.11:g.(127300001_131500000)_(131500001_136400000)dupinv` | refused | — | the `dupinv` spelling for the same event — `class="invalid"` at `inversion.md:19`; rejected at parse (trailing `inv` after `dup`), in every mode. The `ins<range>inv` form above is used instead |
...
an `inv` beside a substitution, and `del` + `inv` + `ins`. Their multi-megabase members are
genuinely distinct events, so there is no separation/merge tension; they corroborate that connected
complex changes are `;`-joined cis alleles of basic types. Descriptive.

## `complex.md:169-175` — Discussion: ad-hoc labels are refused; `N[count]` is legal

> No, not really, it is not exact.

Ferro: three points from the Q&A. Ad-hoc labels (`TSD`, `insL1.603bp`) are not valid HGVS — a
description must be exact — and are refused. The discussion restates the duplication definition
(`complex.md:173`): `dup` is used only when the duplicate sits directly 3′ of the original copy,
corroborating that the `dup` label ranks the change itself, not how the partition was cut.
`N[count]` is the spec's own endorsed spelling for an unknown-sequence insert of known size
(`complex.md:175`) — legal HGVS; the only place ferro refuses an `N`-unit repeat is the HGVS→SPDI
conversion path, where it declines rather than emit a storable-but-wrong triple, leaving the HGVS
spelling untouched.
...
| `NG_012232.1(NM_004006.2):c.123+51_123+52ins[N[603];123+45_123+51]` | conformant | — | the spec's endorsed unknown-insert form (`complex.md:175`): `N[603]` gives the size, the range describes the target-site duplication as an insertion — legal (only the SPDI path refuses an `N`-unit repeat). On a real `c.` reference ferro would expand the same-reference range citation to literal bases, so the output is not the recommended range-citation form — conformant (parse-only) |

## Coverage note (structural)

The normalization corpus exercises essentially none of `complex.md`: multi-megabase genomic
breakpoints, cross-chromosome and ring `::` composites, `sup`/marker chromosomes, ISCN
comma-thousands renderings, and orientation-reversed dup-as-`ins(range)inv` are all outside what
the corpus generator can emit — it has no ISCN/complex axis. A `0` from that corpus here is a claim
about the instrument, not about this file's behaviour; confidence about complex/ISCN normalization
must come from targeted fixtures, not the corpus.
