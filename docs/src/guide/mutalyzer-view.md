...
An axis that is not derivable from the input and reference is shown as `-` (for example, a bare-`NM_` coding input with no genome alignment renders `Genomic:     -`, not an omitted line). The `--transcript` accession is required for a bare genomic input and optional for a `c.`/`n.`/`r.` input that already names its transcript.
