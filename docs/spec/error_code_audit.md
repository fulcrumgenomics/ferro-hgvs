...
- **Registered** — the code is present in `src/error_handling/registry.rs`
  ...
  code — e.g. generic non-ASCII inputs are caught by the dash/quote
...
| W3022 | InitiatorMetCanonicalization — protein insertion (or affix-trimmed delins) whose canonical duplication form includes position 1 (the initiator methionine); emitted as an advisory alongside the canonicalized `p.Met1dup`-form output | [recommendations/general.md](https://hgvs-nomenclature.org/stable/recommendations/general/) §3 (prioritization: dup preferred over ins); [recommendations/protein/variant/deletion.md](https://hgvs-nomenclature.org/stable/recommendations/protein/variant/deletion/) §63-65 (Met1-inclusive deletion); [recommendations/protein/variant/substitution.md](https://hgvs-nomenclature.org/stable/recommendations/protein/variant/substitution/) §45-65 (start-codon substitution rule `p.0`/`p.0?`/`p.(Met1?)`) | Enforced — emitted by `try_protein_ins_to_dup` / `try_protein_delins_canonicalize` in `src/normalize/mod.rs` when the resulting dup interval includes position 1. `mode_behavior` is `warn_accept` (Reject in strict, WarnAndAccept in lenient/silent); the warning is purely informational (consumers should consider the substitution-rule interpretation); the runtime does not attempt to auto-correct the input to an alternative form. Closes [#92](https://github.com/fulcrumgenomics/ferro-hgvs/issues/92) |
...
| W4004 | PositionPastEnd — `c.<N>G>C` / `c.<N>dup` / `c.<a>_<b>del` where the position exceeds the resolved transcript's CDS-end or transcript-end (e.g. `c.946G>C` against an `NM_` whose CDS is 945 bases) | [recommendations/general.md](https://hgvs-nomenclature.org/stable/recommendations/general/) (position numbering must reference an existing base in the transcript) | Enforced — emitted as `NormalizationWarning::PositionPastEnd` from `src/normalize/mod.rs::check_cds_pos_past_end` (c. axis) and `check_tx_pos_past_end` (n. axis), called from `normalize_cds` / `normalize_tx` before the `needs_normalization` short-circuit so substitutions are also checked. Strict mode converts the warning into `FerroError::InvalidCoordinates` in the outer `Normalizer::normalize` wrapper; lenient mode emits the warning and short-circuits to the canonical variant; silent mode skips the check entirely. Covers `c.<N>` against CDS length (bound_kind `cds-end`), `c.*<N>` against the post-CDS transcript suffix (`transcript-end`), `c.-<N>` against the 5'UTR length (`5utr-start`, #348), and `n.<N>` against full transcript length (`transcript-end`, #347). Intronic offsets remain out of scope (#336) |
...
| W5003 | VariantExceedsReference — provider returned fewer bytes than the HGVS interval span; variant references positions outside the reference (e.g. `NG_032871.1:g.32476_53457delins...` when the reference is shorter than position 53457) | [background/refseq.md](https://hgvs-nomenclature.org/stable/background/refseq/#reference-sequences) §43: "the entirety of the variant sequence must be encompassed by the selected reference sequence" | Enforced — emitted as `NormalizationWarning::CanonicalSplitSkipped` from `apply_canonical_split` in `src/normalize/mod.rs` when `ref_bytes.len() != expected_span`. Strict mode promotes to `FerroError::VariantExceedsReference` via `Normalizer::normalize`; lenient warns + preserves; silent preserves without warning. Matches biocommons hgvs which raises `HGVSInvalidVariantError` (#355) |
