...
fn clamp_intronic(iv: &CdsInterval) -> Option<CdsInterval> {
    ...
    // A purely intronic edit (both endpoints carry a non-zero intronic offset) whose
    // endpoints share an exon anchor — e.g. `c.100+5_100+10del` — collapses to a
    // spurious single-base span (`c.100_100del`) that does not exist in the source
    // variant. An intron-only edit has no RNA consequence, so decline instead.
    if start.base == end.base && start_in.is_intronic() && end_in.is_intronic() {
        return None;
    }
...
pub(crate) fn predict_rna(coding: &HgvsVariant, transcript: &Transcript) -> Option<HgvsVariant> {
    ...
    let cds = match coding {
        HgvsVariant::Cds(c) => c,
        _ => return None, // alleles handled in a later task; non-c. inputs unsupported
    };
