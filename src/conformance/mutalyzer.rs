...
pub enum Policy {
    ...
    /// ferro re-derives a multi-member allele's partition from the resulting
    /// sequence; mutalyzer hands back the partition the input was authored
    /// with, 3'-shifted.
    ///
    /// **A different mechanism from
    /// [`Policy::SeparationOneGenomicSplit1616`], which is why it is a separate
    /// policy.** That one is a *split* of one spanning `delins` into individual
    /// descriptions, caused by scoping `DNA/delins.md:47`'s payload-coincidence
    /// carve-out to the coding DNA axis. This one is a *re-derivation* of the
    /// whole member list, caused by deleting the input-relative weight bound:
    /// the old comparand refused any derivation weighing more than the member
    /// list the caller happened to write, so an authored three-member spelling
    /// was handed straight back.
    ///
    /// **The authority is `rulings[derivation-may-not-be-bounded-by-the-inputs-spelling]`
    /// (decided)**, which holds that no clause compares a description to the
    /// *input* — the recommendations compare it to the sequence. Its siblings
    /// `rulings[canonical-form-choice-when-both-legal]` (derive from the
    /// resulting sequence and emit what falls out, not the input's spelling) and
    /// `rulings[separation-is-a-property-of-the-spelling-not-of-the-variant]`
    /// (separation is read off the re-derived partition) make ferro's answer the
    /// ruled *method's* output rather than a competing preference.
    ///
    /// **Both forms are conformant and this is not a conformance claim against
    /// mutalyzer.** Every derived member is separated from its neighbours by at
    /// least one unchanged nucleotide and is described individually, which is
    /// what `general.md:34` asks for; the codon exception (`DNA/delins.md:18` /
    /// `general.md:35`, widened by
    /// `rulings[codon-carve-out-shape-restriction]`) is checked and does not
    /// reach the one separation-of-one pair here, because the two positions fall
    /// in different codons and so do not "together affect one amino acid".
    ///
    /// **Under `rulings[adjudication-precedence-order]` mutalyzer is not an
    /// authority**, so this is registered rather than asserted, and the
    /// `normalized`/`genomic` fields keep MUTALYZER'S recorded answer as the
    /// evidence of what was diverged from. Movement is AWAY from the reference
    /// implementation (#1616).
