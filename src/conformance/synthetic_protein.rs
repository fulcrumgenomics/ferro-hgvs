...
fn insertion_payload(
    ...
    match sequence {
        ...
        ProteinInsSeq::Stop { position } => Ok(Payload::ByCount {
            count: usize::try_from(*position).unwrap_or(usize::MAX),
            terminates: true,
        }),
    }
}
    ...
    match direction {
        ExtDirection::CTerminal => {
            ...
            let length = count
                ...
                .map(|count| start + 1 + usize::try_from(count).unwrap_or(0));
        ...
        ExtDirection::NTerminal => ProteinDenotation::Indeterminate(Indeterminate {
            prefix: String::new(),
            length: count.map(|count| reference.len() + count.unsigned_abs() as usize),
