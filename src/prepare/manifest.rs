...
pub struct ReferenceManifest {
    ...
    /// --ensembl --no-cdot` on an existing reference re-downloads only the
    /// Ensembl cdot, and `--genome grch37` only the GRCh37 cdot, each leaving
    /// the others at whatever release the previous run used. (Plain
    /// `--ensembl` is *not* an example: `--genome` defaults to `grch38`, so it
    /// refreshes the GRCh38 RefSeq cdot too.) One scalar cannot describe a
    /// mixed reference truthfully — it either names a release some artifact
    /// did not come from, or must be dropped, discarding provenance that is
    /// perfectly well-defined per artifact.
    ///
...
mod tests {
    ...
    #[test]
    fn cdot_data_versions_survive_a_save_load_round_trip() {
        // Every other test works on the struct or a hand-built Value, so a
        // serde-name or `skip_serializing_if` mismatch would go unnoticed. Walk
        // the real path: stamp -> save() -> load_or_default().
        use tempfile::TempDir;
        let dir = TempDir::new().unwrap();
        let versions = BTreeMap::from([
            ("cdot_json".to_string(), "data_v0.2.28".to_string()),
            ("ensembl_cdot_json".to_string(), "data_v0.2.32".to_string()),
        ]);
        let mut m = ReferenceManifest {
            reference_dir: dir.path().to_path_buf(),
            transcript_count: 1,
            available_prefixes: vec!["NM_".to_string()],
            cdot_data_versions: versions.clone(),
            ..Default::default()
        };
        m.save().unwrap();
        ...
        let raw = std::fs::read_to_string(dir.path().join("manifest.json")).unwrap();
        assert!(
            raw.contains("cdot_data_versions"),
            "the map must be persisted under its own key: {raw}"
        );
        assert!(
            !raw.contains("\"cdot_data_version\":"),
            "the retired scalar must not be written: {raw}"
        );

        let back = ReferenceManifest::load_or_default(dir.path()).unwrap();
        assert_eq!(back.cdot_data_versions, versions);
        assert_eq!(back.cdot_data_version, None);
    }
