...
//!
//! This module reaches back into `crate::normalize::merge` for one reason: the
//! four legacy wrappers ([`LegacyRuleArm`]) need `merge`-private
//! `partition_block*`/`Piece` internals, so they are defined there and merely
//! *registered* here. It is one of the confined `partition → normalize` seams the
//! [`crate::partition`] module doc enumerates.
