...
/// Reverse complement over the full IUPAC alphabet, `None` on a byte it does not
/// model. Routed through [`crate::sequence::complement_base`] so an `Inv` round-trips
/// under the same definition `merge::is_inversion` used to type it: a span holding an
/// ambiguity code (`R` pairs with `Y`) is an inversion there, so it must apply as one
/// here, and a span holding an unmodelled byte is no inversion at all.
fn revcomp(b: &[u8]) -> Option<Vec<u8>> {
    b.iter()
        .rev()
        .map(|c| crate::sequence::complement_base(*c))
        .collect()
}
