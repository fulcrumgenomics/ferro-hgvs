...
fn run_mutalyzer(
    ...
    let process = |v: &str, writer: &mut dyn Write, line: Option<usize>| -> bool {
        ...
        let tx = match resolve_mutalyzer_transcript(&parsed, transcript) {
            Ok(tx) => tx,
            Err(msg) => {
                // Route through the same format-aware diagnostic stream the parse
                // and projection branches use, so `--format json` keeps a
                // parseable error object instead of this one plain-text line.
                let err = FerroError::UnsupportedProjection { reason: msg };
                let _ = cli_output_error_with_context(&mut io::stderr(), v, &err, out_format, line);
                return false;
            }
        };
        match projector.mutalyzer(
            &pre.preprocessed,
            &tx,
            ferro_hgvs::ProteinRenderStyle::default(),
        ) {
            Ok(mut result) => {
                // Preserve the `--ignore`/`--reject` preprocessor's correction
                // warnings: `projector.mutalyzer` only sees `pre.preprocessed`,
                // so it cannot reconstruct these. The view carries its warnings
                // inline (JSON `warnings` and the text `Warnings:` block alike),
                // so folding them into `result.warnings` surfaces them in both
                // formats — the same list `--ignore` empties, so it stays
                // consistent with `ferro normalize`. Codes namespace-distinguish
                // the preprocessor's `W####` from the normalizer's.
                for w in &pre.warnings {
                    result.warnings.push(ferro_hgvs::project::MutalyzerWarning {
                        code: w.error_type.code().to_string(),
