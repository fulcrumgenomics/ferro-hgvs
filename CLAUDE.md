...
leaving the citation pointing at unrelated prose. Do the same in prose comments: `general.md:33`,
...
| `self-cancelling-across-ring-junctions` | `DNA/complex.md:130` governs: `general.md:57` does **not** reach the `::`-joined segments of a ring. `:57` is a sub-bullet of the prioritisation bullet `:55`, so it inherits its antecedent — it redirects to a preferred single-type description — and `complex.md:5` defines complex as precisely the case where no such description exists. `:130` settles the operators on identical member content: `::` asserts a join that `;` does not, so a `::` composite is not reducible to its member set |
| `projection-codon-exception-is-decided-by-the-rendered-axis` | **2026-08-11.** `DNA/delins.md:42` governs, and reaches **only an axis that declares a reading frame**: a projection does not re-merge its derived genomic axis to match the coding one. `:42` is a conditional whose second conjunct — "together affecting one amino acid" — is unstatable on a genomic reference, so `:17` governs there unopposed and the members stay individual. `general.md:22`/`:25` make the prefix a claim about the **type of reference sequence**, so `LRG_199:g.…` is genomic however gene-scoped its accession; `general.md:43` is precedent for the spec scoping a rule by naming prefixes. Corrects an over-reading of `delins-codon-carve-out-gap-one`, which settles *when* two variants merge and is silent on which axes. Scoped to **DNA axes**: the `r.` axis merges on `RNA/delins.md:18`'s own authority and is not ruled on here; the `n.` axis is left open |
...
| `confluence-gate-is-apply-equality-on-every-determined-axis` | **The gate is restated, not retired.** "Equivalent inputs produce one canonical output" stands, stated non-circularly as "`normalize` is constant on each equivalence class", where equivalence is apply-equality on every *determined* axis (`EquivalenceLevel::CrossAxisSequenceMatch`) and never `NormalizedMatch` — a relation over `apply`, whose codomain is bases, so it cannot collapse into the normalizer it gates. `general.md:42` governs as the spec's statement that one variant's several descriptions are one object; `duplication.md:148` supplies the counterexample making single-axis apply-equality insufficient. Protein is excluded — translation is many-to-one and `p.` states a consequence, not a denotation. Confluence is asserted only over *decided* pairs; the record also states what it does **not** solve, citing `RNA/repeated.md:20-21` and `delins.md:83`: two descriptions can be apply-equal on every axis and still make different epistemic claims, which is a canonical-form question and must not be encoded as a rung |
| `conflicting-member-geometry-refusal-scope` | `DNA/alleles.md:5` governs — the *definition*, not `general.md:57`, is what reaches nested and coincident-insertion geometries; `:57`'s stated ground literally describes only its own `del`+`dup` example. `general.md:55` is cited to record that it does **not** reach a multi-member allele |
| `delins-merge-vs-individual-gap-two-or-more` | `DNA/delins.md:44-47` governs `:17`, **scoped** to the alignment-coincidence shape `:44-47` describes. Where a separation of two or more arises from anything else, `general.md:33` still governs; unscoped the reading reaches roughly fifteen times the row set the argument was made on. **Scoped by DIRECTION too** (2026-08-11): it reaches the **net-deletion** case only — payload shorter than the span — and does **not** reach net insertions, where the split form stays canonical. That keeps `merge.rs`'s `payload.len() < span.len()` gate, whose grounds are the rejection of SVD-WG010 (`:5`), whose `:16` example is itself a net-insertion merge; `DNA/duplication.md:90-92`, which publishes a net insertion as a `[dup;ins]` split; and the fact that the ruling's whole evidence base is net-deletion |
| `delins-payload-coincidence-carve-out-is-coding-dna-scoped` | **Scoped to the coding DNA axis.** `delins.md:47`'s payload-coincidence carve-out reaches `c.` and nothing else; on `g.`/`m.`/`o.`/`n.`/`r.`, `general.md:33` governs and members stay individual. `:47`'s stated reason is preventing "incorrect predictions for the consequences on protein level", which has nothing to bite on elsewhere, and `general.md:42` shows the spec says "ALL descriptions" when it means universal. **`r.` is out in both directions** — a DNA document has no jurisdiction over the RNA axis, and `RNA/delins.md` states no `:47` counterpart, so `RNA/delins.md:17` stands unqualified. Do not gate this on `AxisFrame::reading_frame`: that is true for a coding `r.` too, which is how the defect arose. Counterweight in the record: `:47`'s other ground, simplicity, is axis-neutral. **Honoured on the SHIPPING path as of #1616** — it was honoured only by `payload_coalesce_applies`, which gates a non-shipping partition rule, while `partition_block` applied the carve-out on every axis. What it costs is `g.` rows in the payload-coincidence stratum that the ungated pass would have merged — 380 over the designed cis corpus. It does **not** cost the stored 723-base `g.` `delins` its 193-member re-derivation: that claim was withdrawn as measured false (the row is byte-identical on both arms), and its cause is the `coalesce_whole_block_inversion` family |
| `delins-recommendation-reach-when-the-input-arrives-split` | **2026-08-12. `:46` governs — an INSERTED SEQUENCE is what `:47` reaches.** `:46` states the mechanism the carve-out rests on, and states it as "parts of the **inserted sequence** 'align'", so `:47` reaches a payload-coincidence split only where some derived member supplies bases while consuming a *different* number of reference bases. A split of pure deletions inserts nothing, so nothing re-aligned, so `general.md:33`/`:17` govern it unqualified. It is the only reading under which both worked examples in the passage come out as the spec writes them — `:44-47`'s own example merges (its `895_896delinsC` is gap-bearing) and W58 stays split. A further scope on `delins-merge-vs-individual-gap-two-or-more`, sitting under that record's shape and net-deletion scopes and under the `c.`-only axis scope; it widens none of them. **Not a confluence ruling** — the pass runs last on already-derived pieces, so each class converges either way; this picks *which form*. Already implemented by `split_carries_a_gap_bearing_insert` (#1698) on the `CanonicalCoalesced` arm only, so it moves no shipped row and the real-corpus disclosure is owed by the default flip. Counterweight in the record: `:47`'s other ground, simplicity, is mechanism-neutral and is given no weight — the same second ground the axis scope also declines, so both narrow together if it is ever accepted |
| `inversion-vs-two-delins-76-83` | `inversion.md:5` governs and `c.76_83inv` is one `inv` — **the outcome stands, the reason does not**. Its competitor-type ground (that `general.md:55` ranks substitution but not `delins`, so #1230's substitution case stays split) was superseded on 2026-08-13 by `whole-span-reverse-complement-types-as-inv`; the record carries a supersession paragraph saying which of its own arguments are no longer live. Its base derivations, the `inversion.md:33-34` finding and the mean-versus-distribution refutation of the coincidence-density argument all remain good |
| `whole-span-reverse-complement-types-as-inv` | **UNIFORM, 2026-08-13.** A span whose whole content is replaced by its exact reverse complement is typed `inv` — regardless of how much of the interior coincides, and regardless of whether the competing partition is substitutions, `delins` or anything else. `inversion.md:5` governs, because the property it states is a property of the whole span and has no term for interior columns. **Overturns #1230** and supersedes the competitor-type half of `inversion-vs-two-delins-76-83`. The load-bearing ground is that both halves of the old rule argued over `general.md:55`, which `adjudication-precedence-order`'s already-decided E1 holds "cannot settle a merge-versus-split question at all" — so the clause reaches neither side. Corroborated by the spec's own published `NM_004006.2:c.4145_4160inv`, re-derived as 10 of 16 columns changed with two three-base unchanged interior runs. **A rule 6 choice, not a rule 1 correction**: per #1703's 2026-08-12 reclassification the spec *permits* the `inv` and no clause compels it, so the split forms are conformant and this is the maintainers choosing among them. **Ships no code and moves no row by itself** — it decides moves belonging to #1703, #1541 (`c.[710T>A;713T>A]` → `c.710_713inv`) and #1575 (the **93** sweep rows that still repartition after #1706 absorbed 62 of the original 155 — all 93 the all-substitutions case, so wider than the 25 `delins`-bearing rows #1575 costed and narrower than the 155 the record was first written on), and the real-corpus disclosure is owed by the implementing change. Two carried-over claims are **corrected in the record rather than repeated**: the "one corpus row" costing (stale in both directions — the corpus now varies separation but still not run width, and `long_block_inversion` is the family that reaches the geometry), and the attribution of the 457 nt `NC_000013.10` guard failure to `substitution.md:32` — the guard's 12 pairs reproduce exactly, but all 12 involve an `ins` that consumes no reference and none is the adjacent-substitution shape `:32` names. **Reconciled in the same change:** the narrower `inversion-vs-a-mixed-member-competitor` (2026-08-12) landed first with #1706 and is **subsumed** — it now carries a supersession paragraph retiring its clause (4) carve-out for #1230, its outcome untouched |
| `inversion-vs-a-mixed-member-competitor` | **2026-08-12.** The same question where the competitor is a MIX — lone substitutions beside multi-column members. `inversion.md:5` governs, as a **permission**: the `inv` is conformant, the competitor is conformant too, so this is a `README.md` rule 6 choice and must never be cited as conformance. It deliberately does **not** rest on `general.md:55`, which read plainly argues the other way (every competitor contains a substitution, rank (1), above inversion); the reading that saves it is `conflicting-member-geometry-refusal-scope`'s, that `:55` does not reach a multi-member allele. `general.md:57` was measured and does not reach it either — no competitor contains a `del`, `dup` or `ins`. The ground held is that notation must not turn on base coincidence. **Superseded in part on 2026-08-13** by `whole-span-reverse-complement-types-as-inv`: the outcome stands, but its clause (4) carve-out — that #1230's all-substitutions case is one both gates refuse — does not, and its gate widening is retired rather than contradicted |
| `contiguous-insertion-split-by-a-blocked-derivation` | **NO** — `general.md:33` is stated over "two variants", and this locus carries one. Two spellings of one contiguous insertion are not two variants separated by unchanged nucleotides, so `:33` does not reach them; the split survives because a derivation was blocked, not because a clause requires it. Verified by applying both spellings through `hgvs_to_spdi`, independently of the normalizer |
| `separation-is-a-property-of-the-spelling-not-of-the-variant` | **Read the ruling, not the id** — the title states the *question*, and the answer is the opposite of it. The separation `general.md:33` keys on is read off the partition **re-derived from the resulting sequence**, never off the input's spelling. On its own case both spellings converge on `g.[1001009_1001010del;1001013del]`. Deviates from `delins.md:79-84`, whose provenance rationale ferro cannot honour |
| `separation-rule-force-modal-or-negation` | **RULE 2, whole clause.** `general.md:33`'s "and not" names the excluded alternative; the **modal** grades the clause. Decisive internally: "described individually" and "described as a delins" are complements, so forbidding one would make the other mandatory and leave "should" doing no work — a clause cannot grade its positive half as a preference while forbidding the sole alternative. Confirmed by `DNA/delins.md:81` restating this very rule with "**preferably**", and by the spec pairing "and not" with "must" only where the alternatives are *not* exhaustive (`duplication.md:18`'s "and not as, **e.g.**, an insertion"). The prohibition line the spec does draw is at separation **zero** (`substitution.md:32`, `class="invalid"` + "not correct"). Changes classification only: rule 2 still binds and outranks maintainer judgment, but such an output is a deviation to **disclose and pin with a tripwire**, not a rule-7 bug, so it does not by itself block a release |
...
| `derivation-may-not-be-bounded-by-the-inputs-spelling` | **DELETE THE BOUND.** No clause compares a description to the *input*; the recommendations compare it to the sequence, and `background/basics.md:38`'s design values omit minimality. So `canonicalize_from_sequence` may not refuse a derivation for weighing more than the member list the caller happened to write — that comparand contradicts `canonical-form-choice-when-both-legal` in terms. **Read the scope**: the ruling removes an input-relative comparand, it does **not** license the merges the comparand happened to be blocking. Those are governed by `general.md:33` and by `delins-merge-vs-individual-gap-two-or-more`. **Do not attribute the masking to `best_alignment`'s single-gap restriction (#1617)** — the record measures the producers as `partition_block`'s two payload-coincidence exits plus the `MAX_SPLIT_BLOCK` length cap, and withdraws that earlier attribution by name. **All three are now closed, on the SHIPPING path** (#1616): the carve-out is scoped to `c.` inside `partition_block`, and a block the length cap hands back **unexamined** may not be emitted as one spanning member off that axis — a block nothing looked at is the absence of a finding, not a finding of no separation. `MAX_SPLIT_BLOCK` itself is untouched and raising it stays open. Costs 18 (3') / 14 (5') `spec_conformance_axis` classes their confluence, which rule 2 outranks |
| `unchanged-is-read-over-every-minimal-alignment` | **COLUMN-BASED, OVER EVERY MINIMAL ALIGNMENT, 2026-08-07 — recorded 2026-08-14.** A reference base is unchanged iff **every minimal alignment** matches it; `general.md:33`'s "separated by one or more nucleotides" and `delins.md:16`'s "two or more consecutive nucleotides" are both read against that notion. Decided while building the forced-unchanged-column detector (#1539/#1540) and left in a session log for a week, during which **two agents re-derived it differently within one hour** — this row exists because that is the exact cost the record-every-adjudication policy names. The distinction is cell-based vs column-based: `Dominators::matched_ref()` asks whether one `(ref, alt)` CELL matches on every minimal path, the clause asks whether a reference BASE is unchanged in every minimal alignment; on `GACA -> AGAT` the three cost-3 alignments (matched reference sets `{1,3}`, `{0,1}`, `{0,1}`, intersecting to `{1}`) all match ref offset 1 but to different alt offsets, so the cell notion sees nothing while the base is unchanged either way. **Equal length does NOT imply no indel** — `CAG -> AGA` has edit distance 2 (del + ins), so its two middle bases are unchanged and the changes are described individually; `delins.md:16` has no antecedent because nothing is two CONSECUTIVE CHANGED nucleotides, and `substitution.md:32` does not reach it because its own `GC -> TT` example is one where two substitutions IS minimal. **Falsifies a premise stated three times in `merge.rs`** — that an equal-length block "has no gap to place" so every matched base is a coordinate-wise identity; all three guards key on `reference.len() != result.len()` and so exempt precisely the class that breaks them, which is why `FERRO_PARTITION=live` gives the position-wise answer here and the canonical family does not. **Scope MEASURED 2026-08-14 (#1939)**: the affected class is every equal-length block whose minimal alignment is cheaper than position-wise, and it is **large** — 4.98% of uniform-random equal-length pairs at L=3 and 28.34% at L=6 (exhaustive), 67.60% at L=12 (sampled), so past L~9 it is the majority. **Shipped exposure is nil**: 840 of 840 sampled cases produced indel-bearing members and none a lone spanning `delins`, so the default arm already reads it as this record requires. The residual is entirely `FERRO_PARTITION=live`, and its disagreement is **largest at short lengths** (110/120 at L=3 down to 14/120 at L=12) — the opposite of a length-gated intuition. The record carries the limits; do not quote these figures without them. A generator must key on **edit distance against block length**, not length alone. Not a minimality principle — minimal alignment decides which BASES ARE UNCHANGED, a fact about the sequences, never which description is preferred |
| `inverted-duplication-is-derived-as-ins-range-inv` | **DERIVE IT, 2026-08-14.** `DNA/inversion.md:69` governs: an inverted duplication is described `ins<range>inv`, naming the span the inverted copy came from, and ferro now derives that form instead of expanding the payload to reverse-complemented literal bases. Not a `canonical-form-choice-when-both-legal` question — a clause selects **by name**, and gives BOTH spellings (`g.122_123ins123_234inv` where the source starts at the junction, `g.234_235ins123_234inv` where it ends there) distinguished by which side of the original the copy falls, so implementing one would reintroduce the asymmetry `:69` exists to remove. Restated as a Note in five further places, each rejecting `dupinv`. **Reaches the `r.` axis on RNA's own authority** (`RNA/insertion.md:18`, `RNA/duplication.md:21`), not by stretching a `DNA/` clause. **THE COINCIDENCE FLOOR IS HOUSE POLICY — cite nothing for it.** The spec supplies no minimum: `duplication.md:5` admits "one or more nucleotides". The only length statement in play is `inversion.md:5`'s "**more than one nucleotide**", which excludes a one-base payload definitionally (an earlier draft claiming the spec gave no length statement at all was too strong and is corrected in the record). A flat length minimum was implemented first and **ablated**: 34/19/17 failures at minimums 1/2/3, with sequence-preservation guards surviving every threshold, because in a skewed window a short match is far likelier than `4^-n`. Replaced by a composition-aware chance-match probability, which took collateral to **zero over the test suite** — not over the confluence corpus, where it costs one class (see the record). Its constant is 1/N over the representation corpus and **N is hardcoded to a figure `dump_normalized_corpus.rs` has already retired**, so read it as an order of magnitude (~1e-5) and never as a row count; its 8-declines/9-fires boundary is now pinned, because weakening it 85x had left all 41 tests in the dedicated corpora green. **Wired only in `normalize_genome`**, so `c.`/`n.`/`m.` still emit literals and one inverted duplication spells differently per axis; the long-term home is #1946's render stage. **The blocker was never SPDI** — an earlier draft said the form must not ship ahead of SPDI-side support, which #1966 had already landed; the derived form and its literal resolve to identical SPDI. The real blocker was `src/vcf/from_hgvs.rs` splicing the payload's HGVS spelling into the ALT allele (`ALT=C212_221inv`), a pre-existing defect this derivation made reachable from ferro's own output and which is fixed here by declining. Records a prediction of the implementer's that its own adversarial probe **did not support** |
...
| `coding-axis-merges-are-a-disclosed-general-34-deviation` | **A DISCLOSED RULE-2 DEVIATION, 2026-08-13.** Ferro deviates from `general.md:33` and its axis-local twin `DNA/delins.md:17`; `DNA/delins.md:81` governs, as the clause whose "**preferably**" carries the force (the roles are split because the schema refuses one clause being both authority and departure — `:33` reaches these rows, and that is the ruling). #1616 merges eight coding-axis multi-member alleles whose members are authored two or more unchanged nucleotides apart, and `:33` says they are described individually. Stated plainly rather than reasoned away. The chain is that no member is gap-bearing — all eight are pure deletions, `retained == payload length` in every row — so `DNA/delins.md:47` does not reach them on `delins-recommendation-reach-when-the-input-arrives-split`'s reading (#1760), `DNA/delins.md:17` and `general.md:33` govern unqualified, and `general.md:34`'s codon exception cannot reach a separation of two by its own antecedent. `derivation-may-not-be-bounded-by-the-inputs-spelling` says in terms that it does **not** license the merges its deleted comparand was blocking, so #1616 is no authority for its own rows. `separation-rule-force-modal-or-negation` grades `:33` rule 2, whose remedy is disclosure plus a tripwire — and **the tripwire is #1710's `coding_axis_separation_two_or_more_merges`, pinned at 3** in both directions; ANY change from 3, up or down, must name the clause or ruling that carried it. **Corrected 2026-08-15:** this row and the record both said the pin was re-pinned `0 -> 8`. It never was **on `main`** — there the pin has only ever held 0 (#1710, `c165c939`) and 3 (#1835, `750f2334`). 8 *was* committed as a pin, at `386a0adb` on #1616's own branch `fix/remove-input-relative-weight-bound`, which was closed unmerged and never reached `main`; do not restate that as "never committed anywhere". The 8 is a true measurement with #1616 **applied** to its pre-#1835 base — at #1616's BASE itself (`f17a0895`) the counter read 0 — restated in the present tense by a record written 11h43m after the pin had already moved to 3. The bidirectional guard is `assert_eq!(census, pinned)` at `spec_conformance_axis.rs:1982`; the rise-only `assert!` at `:1963` is just its bespoke message. The record carries the reading that was put to the operator and **declined** — that the separation is read off the derived partition, which is one member, so `:33` has no antecedent — and its falsifier: a row that gains a gap-bearing member leaves the set, so the count is a set to re-derive, not a constant to defend. **That falsifier has fired, and the set was re-derived to prove it (2026-08-15).** Measured on the tree: exactly three merges — two at separation 2 (`s01-c3m-pair-del-sub-p4-sep2` + its `c3p` twin) and one at separation 3 (`s01-c1-pair-inv-del-p4-sep3`) — and they intersect this record's eight named rows in NOTHING. #1835 had already adjudicated those three at `spec_conformance_axis.rs:539-591`: "all three are accounted for and none is a `general.md:33` deviation". **So the disclosed population is EMPTY and the disclosure is vacuous on the current partition rule.** The record is deliberately NOT retired and its status NOT changed — that is an operator act under `adjudication-precedence-order` — but the recommendation is to retire it as superseded or narrow it explicitly to #1616's base. Root cause is recorded and was predicted verbatim at `:593-600`: two open PRs moved one counter to different values, `merge-tree` saw no conflict because the constants sit on different lines, and #1899 landed second and kept its own figure instead of re-measuring |
| `unequal-length-block-a-placed-gap-is-not-a-separation` | **KEEP THE BLOCK WHOLE, ON THE `c.` AXIS ONLY — 2026-08-14, amended 2026-08-15.** A lone, minimal, **unequal-length** net-deletion `delins` whose payload merely coincides with the reference is not split. **Read the axis scope before citing it, because the issue's own reproduction is NOT closed**: the rule is gated on `CoincidenceCarveOut::may_disbelieve_a_separation`, i.e. on `delins-payload-coincidence-carve-out-is-coding-dna-scoped`, so it runs on `c.` and nowhere else. #1610 was filed against `NM_TEST.1:n.2_5delinsAAC`, which is `n.`, and that string still returns `n.[2_3delinsAA;9del]`; what this closes is the same block on `c.`. The `n.`/`g.` half is open, and is pinned as open by `a_lone_unequal_length_delins_is_not_split`, which asserts the defect rather than the fix. The ground is that for an unequal-length block `general.md:33`'s antecedent ("two variants separated by") is *manufactured by the chosen alignment* rather than established, so what remains is a **rule 6** choice — `DNA/delins.md:46` constructs exactly this description and `:47` recommends the span. Never cite it as conformance. **Do not re-derive it from any equal-length contrast**: the earlier text argued from a `README.md` sentence that #1937 has since deleted as false (see `unchanged-is-read-over-every-minimal-alignment`); the length gate's ground is the direction scope of `delins-merge-vs-individual-gap-two-or-more`. **Four conditions**, each with a negative control: net deletion (which also keeps #422/#999 out), every separation exactly one unchanged base, some member supplies bases (W58's all-deletion split stays split), and every other member renders as a `delins`. **Why the gate is not optional**: shipped axis-blind it re-admitted on the frameless axes exactly what #1899's sibling gate excludes, taking `guard_violations` **0 → 5** — the rejected SVD-WG010 shape, a rank-1 regression that rule 2 outranks. **Costs, re-measured armed at base `d055fc10`**: no confluence is lost (`converged` holds at 11 016 / 10 755 and every `split_*` bucket holds); the one census figure that moves is `coding_axis_separation_two_or_more_merges` 3 → 5, adjudicated row by row in `spec_conformance_axis`'s module docs. The 2026-08-14 figures (`converged` 11 016 → 10 959, 2 526 of 94 858 rows, "none previously a fixed point") are **superseded** — that fall was the #1440 weight bound's and #1899 deleted it. Corpus now: **1 798 of 95 614 rows move, 776 of them previously fixed points**, every one on a coding axis. **Does NOT** move the MSH2 `g.` row or retire `delins_equal_vs_unequal_length_discriminator.rs`'s `assert_ne!` guard — an earlier revision claimed both. Pinned by `tests/it/issue_1610_lone_unequal_length_delins.rs` |
| `spdi-n-unit-repeat-refusal` | **REFUSE, 2026-08-16 — a `README.md` rule 5 (silent) house choice, cite nothing for it.** An `N`-unit (or `N`-containing) repeat is refused on the HGVS→SPDI path with `UnrepresentableInSpdi`, not emitted as a run of literal `N`s. The recommendations govern HGVS descriptions and are silent on SPDI conversion, so this is ours, not the spec's. `N` names a **length**, not bases (`background/standards.md:30`: `N` = "A, C, G or T"), so `insN[341]` would emit 341 literal `N`s — a storable-but-wrong SPDI triple a consumer could key read counts on, asserting 341 bases the input never specified. That is the same information content as the `ins<length>` shape (`ins341`, `InsertedSequence::Count`) SPDI already refuses (#1967); per the #1747 precedent (`c.pterdel`/`c.qterdel`/`c.1del`, refused for producing storable-but-wrong triples) a plausible-but-wrong answer is worse than a refusal. Decided from the description alone — no provider changes the answer, which is why the error is `UnrepresentableInSpdi` and not `MissingReferenceData`. **Scope**: only undetermined content refuses. `N` is a legal HGVS symbol (`general.md:47` admits IUPAC-IUBMB symbols) and a concrete-base repeat (`insACGT[3]`, `insA[10]`) still expands normally; the guard sits at the shared expansion leaf (`expand_repeat_unit`) and at the short-form `NaEdit::Repeat` arm, so it reaches inserted-sequence repeats, bracket parts, and `g.pos_posN[n]` alike. Pinned by `src/spdi/convert.rs` |
| `past-cds-end-coordinate-is-non-conformant` | **NON-CONFORMANT, MODE-GATED ON BOTH PATHS — 2026-08-16.** A plain `c.N` with N past `cds_end` names no base the coding axis defines: `numbering.md:21` ends the `c.` axis at the stop codon and `:30` opens the `*` zone the moment it ends, so `c.22` (CDS ending at `c.21`) is out-of-scheme numbering for `c.*1`, not a second name for it. Enforcement is mode-dependent per `absolute-prohibition-enforcement-stage` and applied IDENTICALLY on the lone-position and cis-allele paths: strict REFUSES (`W4004`), lenient REPAIRS to `c.*(N − cds_end)` as a recorded correction (the same `W4004`, kept in the warning vector), silent repairs with the message suppressed. Refused at NORMALIZE not parse, like `c-description-against-an-unresolvable-cds-is-refused`, because whether `c.22` is in range is a REFERENCE fact `parse_hgvs` cannot see. The defect (#2018) was the disagreement: the lone path ran the `normalize_cds` bounds gate while the allele path merged members first and silently remapped the coordinate across the CDS/3'UTR seam in every mode. The fix emits `W4004` on the RAW allele members before the merge, mirroring `detect_insertion_overlaps`'s pre-merge `W5002`. `n.`/`c.-N`/`c.*N` bound checks are unchanged. Pinned by `tests/it/issue_2018_past_cds_end_mode_dependent.rs` |
| `cds-unknown-position-is-refused-at-conversion` | **REFUSE, house choice under `README.md` rule 5's silent limb.** `CoordinateMapper::cds_to_tx` refuses a `c.?` (unknown position) rather than coercing it into a concrete transcript coordinate. `c.?` states the position is UNKNOWN (`general.md:87`, `uncertain.md:12`), so it used to answer `c.1`'s own coordinate (base 0 == `CDS_BASE_UNKNOWN` fell into the 5'UTR branch with zero displacement) — a fabricated position, and through `hgvs_to_spdi` a storable-but-wrong SPDI triple a consumer can persist. Same family as #1747 (`c.pterdel`/`c.1del`, storable-but-wrong triples refused) and `spdi-n-unit-repeat-refusal` (`insN[341]`): a plausible-but-wrong answer is worse than a clean decline. A HOUSE CHOICE, not a clause ruling — the recommendations define what `?` MEANS but say nothing about coordinate conversion, so both refusing and propagating the `?` are consistent and no clause governs; never cite it as conformance. Distinct from the two adjacent reference-frame records: `c-and-n-positions-are-flat-transcript-offsets` is the coordinate SPACE a KNOWN number lives in; `c-description-against-an-unresolvable-cds-is-refused` is the REFERENCE failing to resolve a CDS; this is a DESCRIPTION that carries `?`. `src/convert` is not a watched prefix; the refusal code and its pin test shipped in #1747, and this record (#1910) is the missing adjudication. Pinned by `src/convert/mapper.rs::cds_to_tx_refuses_an_unknown_position` |
| `cross-zone-c-positions-order-by-transcript-coordinate` | **TRANSCRIPT COORDINATE, house choice under `README.md` rule 5's silent limb — 2026-08-17.** Two `c.` positions in different numbering zones (`c.-n` 5'UTR / `c.n` CDS / `c.*n` 3'UTR) are ordered by their flat TRANSCRIPT-SEQUENCE OFFSET, not by the zoned `(Region, base)` scalar whose base counter resets per zone (so `c.72` and the adjacent `c.*1` sit far apart in scalar space while being one nucleotide apart on the transcript). `region_sequence_delta` gives the per-zone affine map — `c.n`->`n+cds_start-1`, `c.-n`->`cds_start-n`, `c.*n`->`cds_end+n` — so `c.-1 < c.1 < c.72 < c.*1`, the order the bases sit in. `background/numbering.md` defines each zone (`:21`/`:29`/`:30`) and the flat `n.` numbering (`:52`) but states no cross-zone comparison, and neither it nor `refseq.md` speaks of cis alleles — so no clause governs and it must never be cited as conformance. This is the ordering counterpart of `c-and-n-positions-are-flat-transcript-offsets` (which decides the SPACE a single known number lives in). Documents the policy the comparison passes already enforce via `region_span_delta` (#1482 sibling visibility, #1508 cross-zone overlap); wiring it into the `canonical-coalesced` merge arm's seam handling is the code work of #1856, whose representation movement is disclosed there, not here. Pinned by the #1482/#1508 cross-zone guards |
| `codon-exception-vs-coincidence-carve-out-precedence` | **CODON-FIRST, house choice under `README.md` rule 6 — 2026-08-24.** On a coding axis the codon exception (`delins.md:18`) and the payload-coincidence carve-out (`delins.md:47`) can both reach the same derived pieces and prescribe DIFFERENT merges of them; both forms are conformant and the spec is silent on composition ORDER, so the maintainers choose. Decided codon-first on three grounds — specificity (C1's antecedent is strictly narrower: gap exactly one + one codon + net-length-preserving), the shared protein-prediction rationale (codon-first keeps the amino-acid unit intact and the carve-out still fires on the residual), and a structural signal weighted as ONE ground (the exception is Notes-section prose while the carve-out lives only in the `:44-47` example NOTE) — NOT because the spec mandates it. The rule-vs-example fact-check is TRUE but insufficient alone: neither clause is RFC-2119 normative and `:47` carries its own bolded "is recommended". Surfaced by the partitioner bake-off (M4); decided after adversarial review. Never cite as conformance. |
| `codon-exception-reaches-the-termination-codon` | **INCLUDE, 2026-08-24.** The codon exception (`delins.md:18`, "together affecting one amino acid") reaches the termination (stop) codon. "One amino acid" reads on the described residue POSITION, and Ter IS a residue position; `:19`'s own NOTE illustrates the rationale with a stop prediction (the `[Lys79*;Lys79Asn]` split, one limb a Ter), and two substitutions inside a stop codon give exactly the conflicting per-substitution predictions the exception exists to prevent. Governing `:18`. Corrects the weaker "Ter could itself become an amino acid" reading (the downstream-residue-count reading runs backwards); rejects an exclude-stop-loss carve-out, which would need to translate the resulting codon inside the partitioner and would drop the case where the rationale is strongest. Scoped to a rendered coding frame. |
| `codon-carve-out-excludes-the-compensated-zero-width-gap-two-pair` | **KEEP THE GUARD, 2026-08-24.** A compensated zero-width pair (`Ins(k,k)+Del(k+2,k+3)`) that shares one codon and is net-length-preserving but sits TWO positions apart falls outside `:18`'s explicit "separated by one nucleotide" threshold, so the parent rule `:17` (describe individually) governs. The fragmentation counter fails: the same `(reference, resulting)` presents position-wise as two ADJACENT substitutions (one delins, no gap), so widening C1 to gap-2 would not restore confluence — it would mint a THIRD spelling and make convergence worse. Declining a merge is sound and stable, so no round-trip/idempotence exposure. Resolves the M4 gap-2 candidate as decided-keep; governing `:17`. |
| `payload-coincidence-separation-and-budget-are-house-choices` | **HOUSE CHOICE, rule 6, 2026-08-25.** The payload-coincidence carve-out's two numbers: `max_separation = 5` is the spec's OWN floor (`delins.md:44`'s worked split has interior gaps 4/5/3, so reproducing its single delins at `:47` needs bridging 5), and the value above the floor is a disclosed open item; `mismatch_budget = 1` is forced from above by the decided codon-first precedence (`< 2`) and from below by the budget-0 degeneracy. Ratified by the partitioner bake-off's constrained sweep, NOT by production's self-conceded unsourced `(8, 1)` and never by spec silence. Every value swept reproduces the identical resulting sequence, so it is a spelling choice; never conformance. |
| `separation-zero-dup-member-is-preserved-not-merged` | **KEEP THE DUP, 2026-09-07.** At separation zero a tandem dup exposed by re-derivation (`canonical-form-choice-when-both-legal`, rule 6) is kept, not merged into a delins: `duplication.md:18` (a describable dup **must** be a dup) governs, DEVIATING from `delins.md:88`'s published `c.2077delinsATA`. Decides the dup limb `delins-adjacent-members-when-both-consume-reference` left explicitly open. Two grounds: (1) `duplication.md:18` requires the exposed dup be kept; (2) uniformity — W43 is base-indistinguishable from the ~2,031 real dup-beside-change rows carrying no worked example, and the only thing singling it out is that the spec DOCUMENTED it (a curation fact, not a sequence property), so `:18` applies uniformly. NOT justified by provenance: `delins.md:88` does not rest on it — `delins.md:83`'s provenance was the split rationale the committee REMOVED (`:89`), which if anything argues for the merge. A disclosed rule-2 deviation, taken as an operator act for the step-9 flip; never conformance. |
| `pure-tandem-expansion-insertion-is-not-repartitioned` | **HOUSE CHOICE, rule 6, 2026-09-07.** A pure insertion whose minimal derivation is itself a tandem repeat expansion (`A` → `AAA`) or a clean single-copy dup of its abutting reference tract is emitted as that one member and NOT re-partitioned by the ruled peel's Part B into a `[sub; dup]` whose substitution lands on a base no minimal alignment changes. No clause ranks a one-member repeat expansion against a two-member `[sub;dup]` of the same bases (`open-issues.md:89-90` calls both correct; `general.md:55` ranks TYPES only), so the choice is the project's — but the direction is FORCED: `confluence-gate-is-apply-equality-on-every-determined-axis` requires one output, and the lone-repeat decline (`merge.rs:4812`) makes converging on `[sub;dup]` unreachable, so the repeat wins (restoring the pre-flip `A[3]`). Does NOT disturb `duplication-must-ranks-the-label-not-the-partition`'s 2026-08-19 #2175 extension (a multi-base motif beside a REAL change, non-zero hull); `unchanged-is-read-over-every-minimal-alignment` is supporting context only. Keyed on the reference tract (frameless) so it fires identically on `g.`/`c.`/`n.`. Never conformance. |
