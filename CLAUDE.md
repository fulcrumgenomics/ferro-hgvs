...
| `mosaic-chimeric-substitution-reference-allele-first` | **REORDER TO REFERENCE-FIRST, 2026-08-16 (#2034).** A two-member mosaic (`/`) / chimeric (`//`) **substitution** allele is emitted reference-allele-first: `c.85T>C/=` → `c.85=/T>C`, and the `//` analogue → `c.85=//T>C`. `substitution.md:49` governs — "the reference is always described first" — with `:47` publishing the target `LRG_199t1:c.85=/T>C` as the spec's own worked example. Non-normative prose, cited as the positive statement of the rule ferro follows, not a MUST; it aligns with `canonical-form-choice-when-both-legal` (derive the form, do not preserve the input's spelling). The variant-first spelling is reordered, not refused — per `absolute-prohibition-enforcement-stage`, normalizing a non-conformant input to the conformant form trades nothing and fixes every mode at once. **Scoped to substitution**, the only shape where reference-first is unambiguous: a range edit (`del`/`dup`) has two competing compact forms (`c.79_80=/del` vs `c.=/79_80del`) the spec does not adjudicate and is left as authored, as are three-plus-member mosaics and a mosaic of two real edits (no reference member). The reference member must be a whole-entity `=` or a position-bound identity CO-LOCATED with the substitution; a non-co-located screened region (`c.85T>C/c.60_70=`) and any uncertain member (`c.(85T>C)/=`) are left as authored, since the rebuild would discard the screened region or restate uncertainty as certainty (#2107). Implemented in `Normalizer::normalize_allele` via `reorder_mosaic_chimeric_reference_first`; idempotent, so it cannot recurse. **A real, one-directional migration** of previously-accepted variant-first inputs; the shape-family corpus has no mosaic/chimeric family, so a count from it is a structural zero. Pinned by `tests/it/issue_2034_mosaic_chimeric_reference_first.rs` |
