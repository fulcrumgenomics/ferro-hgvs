...
| `projection-codon-exception-is-decided-by-the-rendered-axis` | **2026-08-11.** `DNA/delins.md:42` governs, and reaches **only an axis that declares a reading frame**: a projection does not re-merge its derived genomic axis to match the coding one. `:42` is a conditional whose second conjunct — "together affecting one amino acid" — is unstatable on a genomic reference, so `:17` governs there unopposed and the members stay individual. `general.md:23`/`:26` make the prefix a claim about the **type of reference sequence**, so `LRG_199:g.…` is genomic however gene-scoped its accession; `general.md:44` is precedent for the spec scoping a rule by naming prefixes. Corrects an over-reading of `delins-codon-carve-out-gap-one`, which settles *when* two variants merge and is silent on which axes. Scoped to **DNA axes**: the `r.` axis merges on `RNA/delins.md:18`'s own authority and is not ruled on here; the `n.` axis is left open **by this record**, and closed by `noncoding-axis-is-re-derived-on-its-own-reference` below |
| `noncoding-axis-is-re-derived-on-its-own-reference` | **THE PROJECTOR RE-DERIVES, 2026-08-13.** The half `projection-codon-exception-is-decided-by-the-rendered-axis` parked. `general.md:23` governs: the prefix is a mandatory claim about the **type of reference sequence**, and `:28` assigns `n` to a non-coding DNA one — so the `n.` axis is normalized on its own reference instead of inheriting the form the `c.` frame chose and reframing it by CDS-offset arithmetic, symmetric with the genomic axis's `normalize_or_fallback` (#737). The grounds are the parked record's own three re-applied: `DNA/delins.md:42`'s "together affecting one amino acid" is no more statable against a non-coding DNA reference than against a genomic one; the prefix claim; and non-re-derivability (nothing in `LRG_199t1:n.389_391delinsTGG` recovers the codon that justified the merge). **Not scoped to `delins`** — it decides *where* the axis's form is settled, so it closes the repeat-form rows too. The refused direction, making the normalizer treat `n.` on a coding transcript as frame-aware, re-opens #1241. **Cost accepted and stated**: `r.` merges on `RNA/delins.md:18`'s own authority, so one change spells differently on `n.` and `r.` — spec-licensed, the same shape as the `g.`/`c.` divergence the parked record accepts, not a confluence bug. **Moves rows**: `axis_noncoding_idempotent` 20 → 0 over 681 projected axes on the live arm (genomic control 214/0); partition-sensitive, so re-read on the #1835 default flip. Its known unpaid cost is #1860, the projector's transcript cache not reaching the normalizer |
