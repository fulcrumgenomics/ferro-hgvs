...
| `absolute-prohibition-enforcement-stage` | **2026-08-10.** The enforcement stage is **mode-dependent**, uniformly: strict fails at PARSE (strict validates input conformance, not merely parseability); lenient does not validate input conformance and fails only when it cannot NORMALIZE; silent is lenient without messages. A third option the record did not enumerate. It answers the record's own objection to unconditional parse refusal, because rule 1 of the README ruleset is about **output** conformance — accepting a non-conformant input and normalizing it to a conformant output trades nothing. Not implemented: #1630, plus #1627/#1628 for the rule-1 output bugs and #1629 for the silent arm |
