...
| `equal-length-block-column-correspondence-is-unique` | **PROPOSED, AND IN OPEN CONFLICT WITH A DECIDED RECORD — 2026-08-14.** On a block whose reference and result are the SAME LENGTH the column correspondence is UNIQUE, so the changed-column set is a fact about the variant rather than an alignment ferro selects; `DNA/delins.md:15`/`:16`/`:17` then type what falls out. `README.md`'s ruleset states that uniqueness. **What it displaces is MINIMAL EDIT DISTANCE**, which is ferro policy and no clause — on `TTGC -> ATTG` the distance-minimal alignment inserts an `A` and deletes a `C` (distance 2) where the correspondence substitutes three columns (distance 3). **NOT grounded on `general.md:56`, which is how #1878 was filed and is the one argument this record refuses.** **THE CONFLICT: `unchanged-is-read-over-every-minimal-alignment` (decided, on `main` since #1899) answers the SAME shape the other way** — it holds a base unchanged iff every MINIMAL alignment matches it, says in terms that equal length does not imply no indel, and names this record's premise false. `TTGC -> ATTG` and its `CAG -> AGA` are the same balanced ins/del shape, so this record cannot be narrowed clear of it without becoming a no-op. Measured: this change turns `normalize_reparse_invariant::a_del_beside_a_dup_re_spells_instead_of_colliding` and `issue_1284_transcript_axis_collision::a_noncoding_del_dup_collision_is_repaired` — that record's own guards — red. **A RESTORATION otherwise**: `partition_block` (the `live` arm, shipped through v0.13.x) already cuts every equal-length block position-wise; all **2,426** of the rows this moves over the 85,642-row shape-family corpus land byte-identically on that arm's answer, none on a third form, none was a fixed point beforehand, and `inv`-bearing rows rise 11,443 -> 12,201 with 758 gained and 0 lost. **Its convergence cost is GONE at the post-#1899 base** — `cis_confluence_axis` 11,271/11,272, `cis_confluence_nr_axis` 5,635/5,636/5,633/5,634, `reported_confluence_pairs` 9/9, `spec_conformance_axis` 11,016 at 3' and 10,773 at 5' (twenty ABOVE `main`'s pin), every rank-1 figure unchanged — which is the experiment the pre-#1899 draft of this record predicted, run |
