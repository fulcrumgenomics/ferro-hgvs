...
develop --features python,extension-module` whenever you need a module you can import (e.g. to
run `pytest tests/python/`). `extension-module` is a separate feature that a `--features` flag
replaces `[tool.maturin] features` with rather than adds to (see above), so it must be named
explicitly here.
