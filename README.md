...
ferro's normalizer follows seven numbered rules: four about its output and three about procedure. They are stated in one place, [`docs/src/reference/normalization-rules.md`](docs/src/reference/normalization-rules.md), and the code and the ruling ledger cite them as "`normalization-rules.md` rule N".
...
- [A Mutalyzer-style multi-axis view](docs/src/guide/mutalyzer-view.md) — `ferro mutalyzer` resolves one variant across every axis at once; interface-compatible with Mutalyzer, the values are ferro's own
