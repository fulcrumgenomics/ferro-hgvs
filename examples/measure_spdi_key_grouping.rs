...
/// count would report a measurement failure as an API limit — the shape the
/// header of `tests/it/generator_completeness.rs` names: a fallible step whose
/// failure is representable as a legitimate value, so a partial run and a clean
/// run write indistinguishable output.
enum KeyOutcome {
