...
fn main() -> ExitCode {
    ...
    // THROWAWAY spike (issue #2155): compare current `normalize` vs the rederive
    // path (`sequence_normalize`) over the whole synthetic corpus.
...
/// THROWAWAY spike (#2155): route each corpus input through the rederive path
/// (`Normalizer::sequence_normalize`, recommended-form on) and diff against what
/// the current `normalize` produced (`row.output`). Categorize each divergence
/// by member-count delta: fewer members = MERGE, more = SPLIT, same = RESPELL.
/// Core-partition proxy that works on ANY axis: apply the description to its
/// reference (`to_sequences`, transcript-aware), then rederive the denoted
/// (ref,alt) under a genomic accession. The rendered COORDINATES are g. (not
/// mapped back to c./n.), but the MEMBER structure — how many members, what edit
/// types — is exactly the axis-agnostic core partition `from_sequences` produces,
/// which is what the merge/split question is about. Axis-specific post-merges
/// (e.g. the c. codon carve-out) are a separate layer and are NOT captured here.
fn rederive_through(normalizer: &Normalizer<MockProvider>, input: &str) -> String {
    let Ok(variant) = parse_hgvs(input) else {
        return "<parse-error>".to_string();
    };
    let pair = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        normalizer.to_sequences(&variant, 64)
    })) {
        Ok(Ok(p)) => p,
        Ok(Err(_)) => return "<to_seq-declined>".to_string(),
        Err(_) => return "<to_seq-panic>".to_string(),
    };
    let opts = ferro_hgvs::normalize::from_sequences::FromSequencesOptions::default();
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ferro_hgvs::from_sequences("NC_TEST.1", 1, &pair.reference, &pair.alternate, &opts)
    })) {
        Ok(Ok(v)) => v.to_string(),
...
fn member_count(s: &str) -> usize {
    match (s.find('['), s.rfind(']')) {
        (Some(l), Some(r)) if r > l => s[l..r].matches(';').count() + 1,
        _ => 1,
    }
}

fn spike_main(seeds: u32) -> ExitCode {
    let rows = dump(seeds);
    // [total, same, merge, split, respell, skip]
    let mut per_axis: BTreeMap<&str, [usize; 6]> = BTreeMap::new();
    let mut examples: Vec<(String, String, String, String, &'static str)> = Vec::new();
    let bad = |s: &str| s.starts_with('<');
    for row in &rows {
        let direction = match row.direction {
            "5prime" => ShuffleDirection::FivePrime,
        ...
        let normalizer = normalizer_for(row.axis, &row.core, direction);
        let rederived = rederive_through(&normalizer, &row.input);
        let e = per_axis.entry(row.axis).or_insert([0; 6]);
        e[0] += 1;
        if bad(&row.output) || bad(&rederived) {
            e[5] += 1;
            continue;
        }
        // Structural comparison only: strings differ by axis prefix/coords, so
        // categorize by member-count delta (merge = rederive has fewer members).
        let (mc_n, mc_r) = (member_count(&row.output), member_count(&rederived));
        let (idx, label) = if mc_r == mc_n {
            (1, "same-count")
        } else if mc_r < mc_n {
            (2, "merge")
        } else {
    ...
    println!("(structural: member-count of rederive-core vs current normalize)");
    println!("axis\ttotal\tsameN\tmoved\tmerge\tsplit\tskip");
    let mut tot = [0usize; 6];
    for (axis, c) in &per_axis {
        let moved = c[2] + c[3];
        println!(
            "{axis}\t{}\t{}\t{}\t{}\t{}\t{}",
            c[0], c[1], moved, c[2], c[3], c[5]
        );
        for i in 0..6 {
            tot[i] += c[i];
        }
    }
    let moved = tot[2] + tot[3];
    println!(
        "ALL\t{}\t{}\t{}\t{}\t{}\t{}",
        tot[0], tot[1], moved, tot[2], tot[3], tot[5]
    );
    println!(
        "\n-- sample moves ({} shown) : axis | input | normalize | rederive | kind --",
        examples.len()
    );
    for (a, i, n, r, k) in &examples {
        println!("{a} | {i} | {n} | {r} | {k}");
    }
    ExitCode::SUCCESS
}
