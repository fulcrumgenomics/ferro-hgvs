...
impl Axis {
    ...
    /// core-relative.
    /// Enumerated rather than defaulted through a `_` arm: a fifth axis added
    /// here would silently inherit offset 0, which is wrong for any axis drawn
    /// against the padded contig (`m.`, `o.`) and is a silent wrong answer
    /// rather than a compile error.
    fn offset(self) -> usize {
        match self {
            Axis::Genome => PAD_OFFSET,
            Axis::Cds | Axis::Tx | Axis::Rna => 0,
        }
    }
    ...
    /// with `u` for `t`.
    ///
    /// Enumerated rather than defaulted through a `_` arm, for the reason on
    /// [`Axis::offset`]: a future axis with its own alphabet must be a compile
    /// error here, not a silent DNA rendering.
    fn spell(self, bases: &str) -> String {
        match self {
            Axis::Rna => bases
                ...
                .collect(),
            Axis::Genome | Axis::Cds | Axis::Tx => bases.to_string(),
        }
    }
...
fn main() -> ExitCode {
    ...
    // Defensive, and not reachable today: `default_value` means clap always
    // supplies at least one value, and an empty or unrecognised label — `--axes
    // ''` included — exits through the `None` arm above before reaching here.
    // Kept so a change to the argument's clap configuration cannot turn "no
    // axes" into an empty corpus that every downstream pin reads as zero.
    if axes.is_empty() {
