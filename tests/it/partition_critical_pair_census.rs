...
//!
//! **The ruled arm is the shipping default.** `DEFAULT_PARTITION_RULE` is `Ruled`,
//! so the engine fixes pinned here reach released output through
//! `partition_ruled`; the `inv` they realize is the ruling's own owed,
//! separately-disclosed representation change. This census exercises the engine
//! directly and does not assert on the public `normalize` rendering.
//!
...
use ferro_hgvs::partition::adapters::RunInv;
use ferro_hgvs::partition::block_ctx::{BlockCtx, FrameContext, Molecule, Provenance};
use ferro_hgvs::partition::driver::{self, partition_ruled};
use ferro_hgvs::partition::ruled::{run_engine, Authority, Cut, Rule};
use ferro_hgvs::partition::seed::Seed;
use ferro_hgvs::partition::Partition;
use ferro_hgvs::ShuffleDirection;
use std::collections::{BTreeMap, BTreeSet};

use crate::common::fixture_gen::fixture_path;

/// The fixpoint rules, in the driver's registration (priority) order — read from
/// the driver itself, so the census sweeps exactly the system
/// `driver::partition_ruled` runs. The length is a tripwire: a rule added to or
/// dropped from the fixed point changes what "confluent" is a claim about, so it
/// must be a visible edit here too.
fn fixpoint_rules() -> [&'static dyn Rule; 6] {
    driver::fixpoint_rules()
}
...
fn diverges(cut: &Cut, ctx: &BlockCtx, a: &dyn Rule, b: &dyn Rule, all: &[&dyn Rule]) -> bool {
    let (Some(na), Some(nb)) = (
        branch_normal_form(cut, ctx, a, all),
        branch_normal_form(cut, ctx, b, all),
    ) else {
        return false;
    };
    na != nb
...
/// `rule`'s branch at `cut`, closed with the full engine: `None` when the rule's
/// scope declines `ctx` or it does not fire. The single definition both
/// [`diverges`] and the sweep's per-cut cache use, so the witness check and the
/// census cannot disagree on what a branch is.
fn branch_normal_form(
    cut: &Cut,
    ctx: &BlockCtx,
    rule: &dyn Rule,
    all: &[&dyn Rule],
    ...
    rule.apply(cut, ctx)
        .map(|branch| run_engine(branch, ctx, all).to_partition())
}
    ...
    for reference in &seqs {
        for resulting in &seqs {
            ...
            for frame in sweep_frames(reference.len().max(resulting.len())) {
                ...
                // The seed declines only on its grid cap, which no block this small
                // reaches, so an error here is a regression that would otherwise drop
                // the block's critical pairs from the census without a trace.
                let seed = Seed::canonical(&ctx).unwrap_or_else(|e| {
                    panic!(
                        "Seed::canonical declined a swept block: {}->{} [{frame:?}]: {e:?}",
                ...
                for cut in reachable(&seed, &ctx, &rule_slice) {
                    // Each rule's branch, closed with the full engine, computed ONCE
                    // per cut: `None` when the rule's scope declines or it does not
                    // fire. The pair loop then compares cached normal forms, which is
                    // exactly `diverges`, built on the same `branch_normal_form`,
                    // without re-running a branch per pair.
                    let normal_forms: Vec<_> = rules
                        .iter()
                        .map(|r| branch_normal_form(&cut, &ctx, *r, &rule_slice))
                        .collect();
                    let applicable = normal_forms.iter().filter(|nf| nf.is_some()).count();
                    if applicable >= 2 {
                        multi_applicable_visits += 1;
                    }
                    for i in 0..rules.len() {
                        for j in (i + 1)..rules.len() {
                            let (Some(nf_a), Some(nf_b)) = (&normal_forms[i], &normal_forms[j])
                            else {
                                continue;
                            };
                            if nf_a != nf_b {
...
fn the_former_whole_span_pairs_now_resolve_to_a_locked_inv() {
    ...
    // on the RULED arm, which `DEFAULT_PARTITION_RULE` selects — a
    // separately-disclosed representation change.
    ...
    for reference in &seqs {
        for resulting in &seqs {
            ...
            for frame in sweep_frames(reference.len().max(resulting.len())) {
                ...
                let (three, five) = match (
                    partition_ruled(&ctx, ShuffleDirection::ThreePrime),
                    partition_ruled(&ctx, ShuffleDirection::FivePrime),
                ) {
                    (Ok(three), Ok(five)) => (three, five),
                    // A declined block declines in both directions — the decline is a
                    // property of the block, not of the requested placement.
                    (Err(_), Err(_)) => continue,
                    (three, five) => panic!(
                        "partition_ruled declined in only one direction: {}->{} [{frame:?}]  \
                         3'={three:?}  5'={five:?}",
                        String::from_utf8_lossy(reference),
                        String::from_utf8_lossy(resulting),
                    ),
