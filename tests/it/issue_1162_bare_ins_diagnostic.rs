...
fn lenient_and_strict_reject_with_the_same_diagnosis() {
    for input in BARE_INS {
        let lenient = parse_hgvs_lenient(input)
            .err()
            .unwrap_or_else(|| panic!("{input:?} must be rejected in lenient mode"));
        let strict = parse_hgvs_with_config(input, ErrorConfig::strict())
            .err()
            .unwrap_or_else(|| panic!("{input:?} must be rejected in strict mode"));

        // "The same diagnosis" is a claim about the structured code and the
        // rendered text, not merely that both mention the class — compare the
        // `Diagnostic.code` channel downstream tooling reads, then the message.
        for (mode, err) in [("lenient", &lenient), ("strict", &strict)] {
            assert_eq!(
                err.code(),
                Some(ErrorCode::InsertionWithoutInsertedSequence),
                "{input:?} in {mode} mode must carry the W3027 code; got: {err:?}"
            );
        }
        assert_eq!(
            lenient.to_string(),
            strict.to_string(),
            "{input:?} must render an identical message in both modes"
        );
    }
}
...
fn delins_and_dupins_keep_their_own_diagnoses() {
    ...
    // `dupins` keeps its `duplication.md:92` diagnosis. Assert the spec citation
    // and the absence of the W3027 code — matching on "dupins" alone would also
    // pass if the detector had swept it up, since the input contains that text.
    let err = parse_hgvs("NC_000001.11:g.100_101dupinsA").expect_err("`dupins` is rejected");
    assert_ne!(
        err.code(),
        Some(ErrorCode::InsertionWithoutInsertedSequence),
        "`dupins` must not be re-diagnosed as a missing payload; got: {err:?}"
    );
    let msg = err.to_string();
    assert!(
        msg.contains("duplication.md:92"),
        "`dupins` must keep its own spec-cited diagnosis; got: {msg}"
    );
}
