...
//!
//! **The base they are measured against is this branch's, not `main`'s.** This is
//! the tip of the partition-normalizer stack, based on `#1571`
//! (`fix/typing-must-not-widen-a-member`), which carries `#1565`'s
//! `FERRO_PARTITION` default flip. Every counter in this table is unchanged from
//! the pre-stack `main` @ 35de96c8 measurement except `non_idempotent_outputs`
//! at 3', which falls from 7 to 4; the confluence figures move a great deal and
//! their pre-stack column is preserved on [`THREE_PRIME`] and [`FIVE_PRIME`].
//!
...
//! | `non_idempotent_outputs` | 4 | 4 | the output is not its own fixed point |
...
///
/// # Measured on this branch's real merge base, not on `main`
///
/// This branch is the tip of the partition-normalizer stack and is based on
/// `#1571` (`fix/typing-must-not-widen-a-member`), which carries `#1565`'s
/// `FERRO_PARTITION` default flip to `preserve`. The pin is what the corpus
/// measures **there**, because a pin measured somewhere the branch does not
/// reproduce is red on every run and can therefore detect nothing — the
/// instrument would be permanently blind in exactly the way it exists to
/// prevent.
///
/// ## The pre-stack baseline, preserved
///
/// The first measurement was taken on `main` at 35de96c8, i.e. *without* the
/// partition model, and is kept here because a baseline that is overwritten
/// cannot be compared against:
///
/// ```text
///                                  main @ 35de96c8      this branch
/// outputs_denoting_no_sequence                 10               10
/// outputs_leaving_the_transcript              371              371
/// prohibition_violating_outputs                32               32
/// sequence_changed                              4                4
/// non_idempotent_outputs                        7                4
/// converged (of 11,882 families)            9,140            3,509
/// split_two / split_three / split_more  2,435/250/57   7,943/364/66
/// ```
///
/// Every rank-1 validity counter is unchanged row-for-row; `non_idempotent`
/// falls. The whole of the movement is confluence, and it is `#1565`'s — that
/// PR owns the flip and declares it in its `Representation-Change:` trailer,
/// with the 5,631-family delta classified there. `FERRO_PARTITION=live`
/// restores the `main` column above exactly, which is the measurement that
/// attributes the movement rather than assuming it.
///
...
/// partitioner.
///
/// Measured on the same merge base as [`THREE_PRIME`], with the pre-stack
/// `main` @ 35de96c8 column preserved for comparison:
///
/// ```text
///                                  main @ 35de96c8      this branch
/// outputs_denoting_no_sequence                 18               18
/// outputs_leaving_the_transcript                0                0
/// prohibition_violating_outputs                32               32
/// sequence_changed                              0                0
/// non_idempotent_outputs                        4                4
/// converged (of 11,882 families)            8,944            3,747
/// split_two / split_three / split_more  2,696/204/38   7,858/239/38
/// ```
///
/// The 5' direction moves the same way and by a comparable amount, which is the
/// cross-check the two-direction measurement exists for: the movement is the
/// partitioner's, not one shuffle direction's.
...
pub(crate) struct Census {
    ...
    /// Outputs violating an absolute textual prohibition. **Asserted at zero.**
    ...
    /// Outputs whose applied sequence differs from their row's. **Asserted at
    /// zero.**
