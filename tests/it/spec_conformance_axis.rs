...
pub(crate) const THREE_PRIME: Census = Census {
    ...
    // directly above was 0 at the time.
    //
    // #2155 does not move this counter: its domain
    // (`CodingSingleExon | CodingMultiExon`) is disjoint from the negative
    // guard's (`Genomic | NonCodingMultiExon`), so widening the carve-out to
    // reach `g.`/`m.`/`n.` cannot touch a counter whose every row is `c.`.
    // Unmoved at 5, measured.
