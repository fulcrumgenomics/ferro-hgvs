...
fn a_del_beside_a_dup_re_spells_instead_of_colliding() {
    ...
    //
    // **The residue is one spanning `delins`, and the authority is the
    // supersession of `unchanged-is-read-over-every-minimal-alignment` by #2174.**
    // The two candidate readings disagree:
    //
    //     position-wise:            C->A, A->G, G->A            cost 3
    //     del C, A=A, G=G, ins A                                cost 2   <- minimal
    //
    // `unchanged-is-read` originally judged "unchanged" against the *minimal*
    // alignment — every cost-2 alignment matches ref offsets 1 and 2, so `262`
    // and `263` read as unchanged and the members stayed individual. #2174
    // scopes that: on an **equal-length run with no interior zero-shift fixed
    // point** (no base surviving at its own coordinate — the position-wise
    // C/A, A/G, G/A all differ), the minimal-alignment interior matches are
    // *shift artifacts*, not separators, so the run collapses to one spanning
    // `delins`. `general.md:34`'s separation therefore does not reach it, and the
    // result respells to `g.261_263delinsAGA`. This is the reading an earlier
    // revision of this branch pinned and the record then withdrew; #2174 restores
    // it, deliberately, and the record now carries the superseding scope.
    assert_eq!(
        normalized, "NC_TEST.1:g.261_263delinsAGA",
        "the colliding `262dup` is re-spelled, and the residual equal-length run \
         has no zero-shift fixed point, so it collapses to one spanning delins"
    );
