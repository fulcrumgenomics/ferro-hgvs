...
//! doc comment). `the_carve_out_merges_on_c_and_n_regardless_of_reading_frame`
//! (renamed from `the_rule_is_scoped_to_the_coding_dna_axis`, whose name now
//! asserts the opposite of the widened scope) pins the WIDER
...
/// The block on `c.`, on `n.`, and on a transcript with no CDS at all — all
/// merge, so the carve-out keys on the axis prefix and not on whether a reading
/// frame is present.
///
/// All three use the same core, the same span and the same payload, so the axis
/// is the only thing that varies. Pre-#2155 the scope was
/// `rulings[delins-payload-coincidence-carve-out-is-coding-dna-scoped]` —
/// `delins.md:47` reached `c.` and nothing else — and this test pinned that
/// `c.`/`n.` disagreement. #2155 supersedes the scope to every DNA axis, so all
/// three rows now merge; what remains for this test to exhibit is the
/// reading-frame-independence (`c.` coding vs `n.` coding vs `n.` CDS-less).
///
/// The `g.`/`m.` axes are pinned in `issue_2155_payload_coincidence_all_dna`,
/// whose contig-backed provider can serve them; this file's transcript-only
/// helper cannot, and duplicating that fixture here would only re-enumerate what
/// that test already covers.
#[test]
fn the_carve_out_merges_on_c_and_n_regardless_of_reading_frame() {
