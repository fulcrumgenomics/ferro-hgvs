...
//! 3. [`generators_that_normalize_stamp_their_error_mode_or_are_allowlisted`] —
//!    a generator that *also* looks like it takes a normalization config must
//!    stamp the error mode it measured under, or name itself in
//!    [`MODE_STAMP_EXEMPT`] against #1629. Same failure family, one step over:
//!    the ledger asks "did the pass see everything it should have?", this asks
//!    "under what precondition were these numbers taken?" — and a census
//!    compared against one from a different mode is as wrong as one built from a
//!    partial pass, with nothing in either file to tell them apart.
//!
...
// ---------------------------------------------------------------------------
// 3. The error-mode ratchet (#1629)
// ---------------------------------------------------------------------------

/// Generators that normalize and write an artifact without stamping the error
/// mode they measured under, each recorded against #1629.
///
/// **Why the list exists.** A census is a measurement through a normalizer, and
/// the normalizer's [`ErrorMode`](ferro_hgvs::error_handling::ErrorMode) changes
/// the numbers: `corpus_prohibited_inputs.rs` re-measures three of the spec
/// corpus's four refusal counters in strict mode and two of them move. So two censuses taken
/// under different modes are not comparable, and before this ratchet **no
/// artifact in the repository said which mode produced it** — the decided ruling
/// `bare-transcript-intronic-position` has to end with the sentence "so that
/// counter is a lenient-mode figure" because the artifact could not.
///
/// The trap is quiet in the expensive direction: `NormalizeConfig::default()`
/// substitutes `ErrorConfig::lenient()`, while `ErrorConfig::default()` and
/// `ErrorMode`'s own `#[default]` are both **strict**. A generator written
/// `Normalizer::new(provider)` therefore measures in lenient mode while reading
/// as though it took the default. Every row below is in that state.
///
/// **This list may only shrink**, on the same two directions and with the same
/// asymmetry as [`LEDGER_EXEMPT`]: a deleted or no-longer-writing generator is a
/// fact, and a generator that now looks stamped is a heuristic. Confirm before
/// deleting a row on the second.
///
/// **Migrating these is deliberately not this change's job.** Six of them write
/// **committed** fixtures whose readers use `#[serde(deny_unknown_fields)]`
/// (`WindowFixture`), so adding the field means regenerating fixtures that need
/// a prepared reference — a change with its own blast radius and its own review.
/// The two artifacts stamped here (`hgvs_spec_normalization.json` and
/// `hgvs_spec_enumeration.json`) are gitignored and regenerated by CI, so they
/// cost no fixture churn at all. Adding a row is the legitimate "considered and
/// declined" answer; silence is what fails the build.
const MODE_STAMP_EXEMPT: &[(&str, &str)] = &[
    (
        "examples/dump_confluence_divergences.rs",
        "#1629: pre-existing; its artifact is a bare JSON array with no document \
         header at all — it does not even record `--direction`, so a mode stamp \
         needs a header first",
    ),
    (
        "examples/dump_normalized_corpus.rs",
        "#1629: pre-existing; the representation-change harness writes TSV whose \
         header is matched byte-for-byte on read, so a mode column is a breaking \
         format change",
    ),
    (
        "examples/extract_biocommons_windows.rs",
        "#1629: pre-existing, and the sharpest case — it switches to \
         `ErrorMode::Strict` per row when `case.expects_error`, so its artifact is \
         MIXED-mode and a single document stamp would misdescribe it",
    ),
    (
        "examples/extract_case_harvest_windows.rs",
        "#1629: pre-existing; writes a committed `WindowFixture`, which is \
         `deny_unknown_fields`",
    ),
    (
        "examples/extract_inversion_sweep_windows.rs",
        "#1629: pre-existing; discloses `--error-mode lenient` in a PROSE COMMENT \
         in `cases.tsv` that nothing parses — the nearest thing to a stamp in the \
         tree, and an argument for a real field rather than against one",
    ),
    (
        "examples/extract_mutalyzer_genomic_windows.rs",
        "#1629: pre-existing; writes a committed `WindowFixture`, which is \
    ...
    (
        "examples/generate_cis_confluence_corpus.rs",
        "#1629: pre-existing; applies edits directly and builds no normalizer — \
         the mode enters downstream in `cis_confluence_axis.rs`, so the stamp \
         belongs on that census rather than on this artifact",
    ),
];

/// Whether a generator's compiled sources *look like* they take a normalization
/// or error-handling configuration — i.e. whether "which mode was this measured
/// under?" is a question its artifact should answer.
///
/// A substring scan, like everything else in this file. A **false positive**
/// (a generator that names `ErrorConfig` in a doc comment) costs one
/// [`MODE_STAMP_EXEMPT`] row and nothing else, which is the right way round for
/// a floor. A **false negative** — a generator that normalizes through a helper
/// naming none of these tokens — is invisible here, exactly as the ledger
/// ratchet's misses are.
fn appears_to_normalize(sources: &[(PathBuf, String)]) -> bool {
    const CONFIG_IDIOMS: &[&str] = &["Normalizer::", "NormalizeConfig", "ErrorConfig"];
    sources
        .iter()
        .any(|(_, text)| CONFIG_IDIOMS.iter().any(|idiom| text.contains(idiom)))
}

/// Whether a generator's **own entry point** looks like it stamps the mode into
/// its artifact — an import of the module, the type name, and the one
/// constructor, all in that file.
///
/// Entry point only, and all three tokens, for the reasons spelled out on
/// [`appears_to_route_through_the_ledger`]: this is the half of the ratchet that
/// *deletes* a row, so it must be hard to satisfy by accident. `ErrorModeStamp`
/// has exactly one public constructor (`of`), and it takes the `&ErrorConfig`
/// the generator actually built — so unlike a free-text field, the cheapest way
/// to satisfy this predicate is to stamp the real thing.
fn appears_to_stamp_the_error_mode(sources: &[(PathBuf, String)], entry_point: &Path) -> bool {
    let Some((_, text)) = sources.iter().find(|(path, _)| path == entry_point) else {
        return false;
    };
    let imports_the_module = text
        .lines()
        .map(str::trim)
        .any(|line| line.starts_with("use ") && line.contains("conformance::error_mode_stamp"));
    let names_the_type = text.contains("ErrorModeStamp");
    let derives_it_from_a_config = text.contains("ErrorModeStamp::of(");
    imports_the_module && names_the_type && derives_it_from_a_config
}

/// The stamping predicate is the row-deleting half, so pin what must not read as
/// adoption — the same shapes that fooled the ledger predicate's first revision.
#[test]
fn a_mention_of_the_stamp_does_not_read_as_stamping() {
    let entry_point = PathBuf::from("examples/plausible.rs");
    let stamps = |text: &str| {
        appears_to_stamp_the_error_mode(&[(entry_point.clone(), text.to_string())], &entry_point)
    };

    // A doc link naming the module and the type, and nothing built from a config.
    assert!(!stamps(
        "//! Provenance lives in `conformance::error_mode_stamp` (`ErrorModeStamp`).\n\
         fn main() {}\n"
    ));
    // An import and the type, but the value invented rather than derived — the
    // exact failure a free-text `error_mode: \"lenient\"` field would allow.
    assert!(!stamps(
        "use ferro_hgvs::conformance::error_mode_stamp::ErrorModeStamp;\n\
         fn main() { let _s: Option<ErrorModeStamp> = None; }\n"
    ));
    // Real adoption: import, type, and the constructor that reads the config.
    assert!(stamps(
        "use ferro_hgvs::conformance::error_mode_stamp::ErrorModeStamp;\n\
         fn main() { let s = ErrorModeStamp::of(&config.error_config); dbg!(s); }\n"
    ));
}

/// A **heuristic** ratchet, not a proof — read [`appears_to_normalize`] and
/// [`appears_to_stamp_the_error_mode`] before quoting what it guarantees. What
/// it buys is that a generator whose numbers depend on the error mode cannot be
/// added without either stamping the mode or saying, in this file, why not.
#[test]
fn generators_that_normalize_stamp_their_error_mode_or_are_allowlisted() {
    let exempt: BTreeMap<&str, &str> = MODE_STAMP_EXEMPT.iter().copied().collect();
    assert_eq!(
        exempt.len(),
        MODE_STAMP_EXEMPT.len(),
        "MODE_STAMP_EXEMPT has a duplicate entry",
    );

    let mut undeclared = Vec::new();
    let mut stale = Vec::new();
    let mut covered = BTreeSet::new();
    let mut measured_generators = 0usize;

    for source in generator_sources_on_disk() {
        let key = source.display().to_string();
        let sources = compiled_sources(&source);
        let in_scope = appears_to_write_an_artifact(&sources) && appears_to_normalize(&sources);
        let stamped = appears_to_stamp_the_error_mode(&sources, &source);
        if in_scope {
            measured_generators += 1;
        }
        match exempt.get(key.as_str()) {
            Some(_) => {
                covered.insert(key.clone());
                if !in_scope {
                    stale.push(format!(
                        "{key}: no longer looks like a generator that both writes an \
                         artifact and takes a normalization config"
                    ));
                } else if stamped {
                    stale.push(format!(
                        "{key}: its entry point now imports \
                         `conformance::error_mode_stamp` and calls `ErrorModeStamp::of`"
                    ));
                }
            }
            None if in_scope && !stamped => undeclared.push(key),
            None => {}
        }
    }

    // VACUOUS-guard, in the spirit of the corpus censuses: a predicate that
    // matched nothing would pass every assertion below while measuring nothing,
    // which is precisely the "structural zero" this repo keeps rediscovering.
    assert!(
        measured_generators > 0,
        "no generator matched `writes an artifact && takes a normalization config`, so \
         this ratchet is measuring nothing — the idiom lists have gone stale",
    );
    ...
    assert!(
        deleted.is_empty(),
        "MODE_STAMP_EXEMPT names generators that no longer exist; delete these rows: {deleted:?}",
    );
    assert!(
        stale.is_empty(),
        "MODE_STAMP_EXEMPT is shrink-only, and these rows look like they are no longer \
         needed. Both checks are substring heuristics, so confirm the generator really \
         does stamp the mode it measured under before deleting the row:\n  {}",
        stale.join("\n  "),
    );

    assert!(
        undeclared.is_empty(),
        "these generators look like they normalize and then write an artifact without \
         recording the error mode they measured under. A census taken under a different \
         mode is not comparable and nothing in the file would say so. Either stamp it — \
         `ErrorModeStamp::of(&config.error_config)`, beside the artifact's other identity \
         fields — or add a row to MODE_STAMP_EXEMPT in this file saying why not: \
         {undeclared:?}",
    );
}
