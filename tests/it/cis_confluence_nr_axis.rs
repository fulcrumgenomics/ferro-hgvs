...
fn the_nr_corpus_is_a_dense_set_of_real_confluence_classes() {
    ...
    // This corpus must be the `n,r` one and nothing else — if a stale `g,c`
    // artifact were installed under this path the censuses below would measure
    // zero classes and pass every "must only go down" pin trivially.
    assert!(
        corpus
            .classes
            .iter()
