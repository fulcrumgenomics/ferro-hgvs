...
use crate::defect_371_transcript_exit::{
    exons_with_run_at_exon1_end, intron_leading_with, junction_provider, CODING,
};
...
///
/// Built from `defect_371_transcript_exit`'s OWN generators rather than
/// hand-rolled here. The bytes agreed when this was a local copy, but nothing
/// pinned them to agree: an edit to those generators would have left this
/// neighbourhood on the old shape, both files green, and this module's
/// oracle-scope claim no longer describing the exemplar that module dissects.
fn exit_provider(strand: Strand) -> ferro_hgvs::MockProvider {
    ...
    junction_provider(
        strand,
        [exons[0].as_str(), exons[1].as_str(), exons[2].as_str()],
        &intron,
        true,
    )
}
