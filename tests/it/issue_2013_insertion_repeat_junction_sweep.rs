...
fn the_reported_allele_preserves_its_denoted_sequence() {
    ...
    // The other public exit, `normalize_with_diagnostics()`, reaches the same
    // `demote_repeats_spanning_siblings` flow through `normalize_core_checked`
    // (#1382), so the sequence-preservation guard must hold there too (#2013
    // review).
    let variant: HgvsVariant = parse_hgvs(&local_desc(body)).expect("input parses");
    let diag = Normalizer::with_config(
        provider(),
        NormalizeConfig::default().with_error_mode(ErrorMode::Strict),
    )
    .normalize_with_diagnostics(&variant)
    .expect("input normalizes");
    let diag_out = diag.result.to_string();
    assert_eq!(
        diag_out, out,
        "the diagnostic exit disagrees with normalize()"
    );
    let diag_after =
        apply_with(&p, HG38_WINDOW, &diag_out).expect("diagnostic output denotes a sequence");
    assert_eq!(
        before, diag_after,
        "normalize_with_diagnostics changed the denoted sequence for {body}",
    );
}
