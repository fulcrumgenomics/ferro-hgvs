...
/// intron.
fn exons_with_run_at_exon1_end(base: char) -> [String; 3] {
...
fn intron_leading_with(lead: &str) -> String {
...
/// first base every time. Sixteen placements, zero exits.
///
...
fn the_five_prime_direction_never_leaves_the_exon() {
    ...
    for base in ['A', 'C', 'G', 'T'] {
        ...
        let blocks = [exons[0].as_str(), exons[1].as_str(), exons[2].as_str()];
        // An intron ENDING in the run's base, so a 5' shift over the unspliced
        // neighbourhood would travel `c.11-1`, `c.11-2`.
        let mut intron = intron_leading_with("CG");
        intron.replace_range(28..30, &std::iter::repeat_n(base, 2).collect::<String>());
        ...
        for strand in [Strand::Plus, Strand::Minus] {
            ...
            // Record the exit BEFORE pinning the string. The other order made
            // `exits` dead code: the exact-string assert fires first on any
            // output that left the exon, so the collection could never be
            // non-empty and its `is_empty()` assert could never report the
            // property this test's module doc cites it for. Collect first, then
            // pin.
            if output.contains('-') && !output.contains(":c.-") {
                exits.push(output.clone());
            }
            assert_eq!(
                output,
