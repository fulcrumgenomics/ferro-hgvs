...
//!
//! `tests/it/cis_spelling_confluence_gap.rs` pins two *other* currently-diverging spelling
//! pairs (#1296, #1304) on the same `cis_apply_oracle` + `padded()` machinery and the same
//! assert-then-flip-red contract as this file, plus eleven pairs that have already
//! converged. Those two are also things the sequence-first rewrite must converge, and will
