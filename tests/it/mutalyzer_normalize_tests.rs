...
///
/// The identity computation itself lives in
/// [`ferro_hgvs::prepare::identity`] (#1001) — promoted there so `ferro prepare`
/// and the loader can share the same definition. This wrapper just reads the
/// manifest this test suite is pinned to and delegates.
fn reference_identity() -> Option<String> {
    ...
    ID.get_or_init(|| {
        ...
        Some(ferro_hgvs::prepare::identity::reference_identity(
            base_dir, &manifest,
        ))
    })
    .clone()
...
fn lrg_noncoding_axis_names_the_transcript_it_was_measured_against() {
    let (Some(vp), Some(p)) = (variant_projector(), provider()) else {
        eprintln!("lrg_noncoding_axis_names_the_transcript_it_was_measured_against: skipping — no manifest");
        return;
    };
