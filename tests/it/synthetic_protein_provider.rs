...
fn the_served_protein_is_the_served_transcripts_translation() {
    for shape in ProteinRefShape::all() {
        ...
        // And the CDS really does encode it: last codon is a terminator, and
        // the rest translate to the peptide.
        assert_eq!(
            built.cds().len(),
            (requested.len() + 1) * 3,
...
fn spec_repeat_counts_agree_with_the_deletion_and_duplication_they_replace() {
    ...
    assert_eq!(sequence(&built, "Ala2[9]"), sequence(&built, "Ala11del"));
    assert_eq!(
        sequence(&built, "Ala2[12]"),
        sequence(&built, "Ala10_Ala11dup")
    );
}
