...
Per the #253 audit, HGVS v21 does not forbid a boundary-spanning del with cross_boundaries=false; ferro accepts and is spec-compatible (e.g. NM_001166478.1:c.59_61del). A terminal accepted divergence from biocommons. Member rows are pending manifest-backed seeding (#325).
...
Per #404, an insertion spanning the CDS-start boundary canonicalizes to a spanning dup by edit-type priority (e.g. NM_000051.3:c.-2_-1insCA). An accepted divergence; member rows are pending manifest-backed seeding (#325).
